...
The base learning rate is $0.01$ with dimensional compensation: for $D$-dimensional data, the effective learning rate is scaled by $D^{0.8} \times P_D / P_2$, where $P_D$ is the number of parameters per splat in $D$ dimensions (chosen empirically to maintain stable convergence across dimensionalities).

The loss function is:
\begin{equation}
\mathcal{L} = \mathcal{L}_\text{recon} + \lambda_a \|\mathbf{a}\|_1 + \lambda_d \|\text{diag}(\mathbf{L})\|_1 + \lambda_b \mathcal{L}_\text{boundary}
\end{equation}
where $\mathcal{L}_\text{recon}$ is the L1 reconstruction loss between the predicted and target volumes, $\lambda_a = 0.1 \times \text{lr}$ and $\lambda_d = 0.01 \times \text{lr}$ (with $\text{lr} = 0.01$, yielding $\lambda_a = 10^{-3}$ and $\lambda_d = 10^{-4}$) regularize splat amplitude and extent, and $\mathcal{L}_\text{boundary}$ penalizes splats whose centers fall outside the volume.

\paragraph{Dynamic operations.}
Every 50 iterations, the optimizer performs dynamic topology adjustment: (1) splats with amplitude below a threshold ($10^{-4}$ of the volume maximum) are pruned; (2) residual-guided relocation moves the lowest-amplitude splats to positions of peak reconstruction error; (3) Z-order (Morton code) sorting is applied to improve GPU cache locality.
Conservative culling retains splats accounting for 99.9\% of cumulative amplitude, removing only the weakest contributions.
...
Splat amplitudes are modulated by a cosine (Hann) window $w(k) = \frac{1}{2}(1 - \cos(\pi k / (L+1)))$ for $k = 1, \ldots, L$, where $k$ indexes voxels within the overlap region, applied along each overlap face, producing a partition-of-unity: adjacent windows sum to 1.0.
...
We also evaluated progressive multi-pass fitting, where a fixed splat budget is distributed across sequential passes of increasing refinement; results are reported in Supplementary Document~3.
...
Spatial indexing uses compound ordering: discrete non-spatial dimensions (time, channel) form the most-significant bits, followed by a space-filling curve over spatial coordinates. Both Hilbert and Morton (Z-order) curves are supported; Hilbert provides superior spatial locality and is the default for serialization, while Morton is used for in-memory reordering during fitting due to its lower computational cost.
...
For each of the 12 datasets, we fitted Gaussian splat models at 10 splat counts geometrically spaced (doubling from 1K to 512K).
...
PSNR was computed as $10 \log_{10}(1 / \text{MSE})$ on volumes normalized to $[0, 1]$ (equivalent to the standard $10\log_{10}(\text{MAX}^2/\text{MSE})$ with $\text{MAX} = 1$); MSE is averaged over all voxels including background.
SSIM\cite{wang2004image} was computed with a window size of 7 and Gaussian weighting.
The compression ratio is defined as $N_\text{voxels} / (K \times F_D)$, where $K$ is the number of splats after culling and $F_D = D + D(D{+}1)/2 + 1$ is the number of float parameters per splat (10 for 3D: 3 center coordinates, 6 Cholesky elements, 1 amplitude).

\paragraph{Blind-spot cross-validation.}
We adapted the blind-spot cross-validation framework of Batson \& Royer\cite{batson2019noise2self} for Gaussian splat model selection.
...
We compared Gaussian splats against H.265 video compression (libx265, treating Z-stacks as video sequences) using the same blind-spot cross-validation framework\cite{batson2019noise2self} applied identically to both methods.
...
OpenCell spinning-disk confocal\cite{cho2022opencell}: MAP4-GFP (microtubules) and Hoechst (nuclei), 51$\times$600$\times$600 voxels; LMNB1-GFP (nuclear envelope) and Hoechst (nuclei), 112$\times$600$\times$600 voxels.
...
Cells3D (HeLa, confocal): nuclei and membrane channels, 60$\times$256$\times$256, via scikit-image\cite{vanderWalt2014scikit}.
...
Mouse heart nuclei (confocal): cropped to ${\leq}$100~million voxels, from the Acto3D project.
