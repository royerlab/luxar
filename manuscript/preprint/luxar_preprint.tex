...
\twocolumn[
    ...
    Systematic evaluation on 12~microscopy datasets achieves 13--313$\times$ compression at 28--43~dB PSNR---a 100-million-voxel light-sheet volume shrinks from 400~MB to 1.3~MB while preserving structural fidelity. A Noise2Self analysis provides a principled criterion for determining when the model has captured all recoverable signal, separating structural information from noise.
...
Desktop tools such as napari\cite{sofroniew2022napari}, Fiji\cite{schindelin2012fiji}, and BigDataViewer\cite{pietzsch2015bigdataviewer} provide powerful visualization but require local installation and data download.
Web-based viewers like Neuroglancer\cite{cheng2024neuroglancer} stream voxel data over HTTP but remain limited to axis-aligned slicing and require multi-resolution pyramid preprocessing.
...
Gaussian splatting\cite{kerbl2023gaussian}, originally developed for real-time novel-view synthesis as an alternative to neural radiance fields\cite{mildenhall2021nerf}, represents a volume as a mixture of oriented, anisotropic Gaussian primitives, placing representational capacity only where signal exists.
...
We adapt Gaussian splatting for microscopy with a purpose-built optimization pipeline featuring asymmetric loss, per-splat Adam optimization with dynamic topology, and Cholesky-parameterized covariance.
We validate the approach on 12~datasets spanning confocal, spinning-disk, and light-sheet modalities, and show via a Noise2Self\cite{batson2019noise2self} analysis that the noise level sets an upper bound on useful model complexity---providing a principled criterion for separating signal from noise.
...
\luxar is designed around a simple principle: scientists should describe their data in Python and explore it in a browser, with nothing in between.
...
An asymmetric L1 loss penalizes over-prediction 10$\times$ more than under-prediction, reflecting the asymmetry of additive models.
...
We evaluated reconstruction across 12~datasets spanning spinning-disk confocal (OpenCell\cite{cho2022opencell}), standard confocal (kidney tissue, intestinal organoid, \emph{C.~elegans} embryo), and light-sheet (Tribolium embryo) modalities, with volumes from 4~to 100~million voxels (Table~\ref{tab:summary}).
...
At 32K~splats, compression ratios range from 13$\times$ to 313$\times$ with 28--43~dB PSNR (Fig.~\ref{fig:ratedistortion}c, Table~\ref{tab:summary}), and fitting completes in 10--60~seconds on a consumer GPU.

\paragraph{Noise2Self identifies the boundary between signal and noise.}
...
We used a Noise2Self\cite{batson2019noise2self} blind-spot analysis to detect this transition: 5\% of voxels were replaced with their donut-median neighborhood values (blind-spot), splats were fitted to the modified volume, and reconstruction quality was evaluated on the masked voxels against the original (pre-replacement) values.
...
Across datasets, the noise level caps useful model capacity (Fig.~\ref{fig:ratedistortion}f): noisier datasets reach their representational ceiling at fewer splats because noise obscures the remaining recoverable signal.
...
This dataset was produced by multi-view deconvolution\cite{preibisch2014efficient}, a process that introduces spatial correlations in the residual noise, violating the pixel-independence assumption underlying Noise2Self.
...
Tiles are embarrassingly parallel: they distribute across GPUs, cluster nodes, or Slurm array jobs---a 512$^3$ volume that would exceed consumer GPU memory can be processed as overlapping 256$^3$ tiles in minutes.
...
The bioimaging community, and scientific computing more broadly, has converged on Python as the common language---from acquisition control and analysis pipelines to machine learning frameworks.
...
This stands in contrast to existing approaches that either require desktop software (napari\cite{sofroniew2022napari}, Fiji\cite{schindelin2012fiji}) or web-development expertise (custom Neuroglancer\cite{cheng2024neuroglancer} deployments).
...
In benchmarks on an NVIDIA RTX~3070, the viewer maintains 60~FPS for scenes up to 500\,000 elements, with uncapped GPU throughput exceeding 2\,000~renders/second for 100K GSplats during interactive camera orbit (Supplementary Table~1).
GSplat scenes load in under one second; larger point-cloud datasets (millions of elements) require longer initial transfer but render at full frame rate once loaded.
...
Our Noise2Self analysis provides a principled way to determine how much representational capacity a given dataset warrants: the information content of the volume drives the splat count upward, while the noise level caps it.
At the optimal count, splats capture the recoverable signal while naturally discarding noise---without an explicit denoising step, unlike dedicated self-supervised methods\cite{krull2019noise2void}.
...
When compared against H.265 video compression using a Noise2Self held-out framework that measures true signal recovery, Gaussian splats achieve 1--2~dB higher held-out PSNR on noisy confocal data at comparable bits per voxel (Supplementary Fig.~3).
...
The same system visualizes millions of stars from astronomical surveys, hundreds of thousands of cells from single-cell atlases (Tabula Sapiens), protein embeddings, super-resolution localizations, and cell tracking trajectories mixing Gaussian splats with polyline overlays---all with native $n$-dimensional navigation (Fig.~\ref{fig:overview}c).
...
Limitations include the inherent approximation of Gaussian splats for thin structures, coordination overhead for very large tiled fits on HPC systems, and the absence of quantitative intensity analysis (the representation is lossy).
The Noise2Self model-selection procedure requires fitting at multiple splat counts, though each fit takes only 10--60~seconds, making a full sweep practical.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figs/fig_visual_comparison.pdf}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figs/fig3_combined.pdf}
    ...
    \emph{Bottom row:} Noise2Self blind-spot validation.
    ...
    \textbf{f,} Optimal splat count (held-out peak) correlates with noise level: noisier data saturates at fewer splats. Tribolium excluded (no held-out peak; see text).}
    \label{fig:ratedistortion}
\end{figure*}

%% ========== TABLE ==========
\input{figs/table_summary.tex}
...
Systematic rate-distortion analysis across all 12~microscopy datasets with Noise2Self model selection, noise floor estimation, per-dataset quality curves, visual reconstructions, and held-out validation.

\vspace{0.5em}
\noindent\textbf{Supplementary Document~3\label{suppdoc:progressive}} --- \textbf{Progressive vs.\ single-pass fitting.}
Comparison at a fixed 32K-splat budget across 12~datasets. Single-pass with conservative culling outperforms progressive approaches for most confocal data.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figs/suppfig_n2s_all.pdf}
\caption*{\textbf{Supplementary Figure~1 $\vert$ Noise2Self analysis for all 7 core datasets.}
Train (solid) and held-out (dashed) PSNR vs.\ splat count.
Stars: held-out peak. Dotted lines: noise floor.
All confocal datasets exhibit overfitting at high counts; the Tribolium light-sheet dataset does not, likely because multi-view deconvolution introduces spatial noise correlations that violate the Noise2Self pixel-independence assumption.}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figs/suppfig_convergence.pdf}
\caption*{\textbf{Supplementary Figure~2 $\vert$ Convergence across datasets and splat counts.}
Most quality is gained within 500--2\,000 iterations.
Model capacity has a larger effect than optimization duration.}
\end{figure*}
...
OpenCell GSplats (102K) & 101\,539 & 0.8 & 60 & 2\,045 & 53 \\
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figs/suppfig_compression.pdf}
\caption*{\textbf{Supplementary Figure~3 $\vert$ Noise2Self-based signal recovery: Gaussian splats vs.\ H.265 video codec.}
Held-out PSNR (Noise2Self, 5\% blind-spot mask) vs.\ bits per voxel for 6 datasets spanning confocal (a--e) and light-sheet (f) modalities.
Blue: GSplats. Orange: H.265 (Z-stack as video, CRF 0--51). Solid: held-out (N2S) PSNR. Faded dashed: raw PSNR (includes noise). Stars: peak held-out PSNR. Dotted: noise floor.
\textbf{For noisy confocal data (a--d), GSplats achieve 1--2~dB higher held-out PSNR than H.265}, meaning superior signal recovery.
...
The GSplat representation is additionally \emph{directly renderable} without decompression.}
\end{figure*}
