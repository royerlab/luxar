...
We also find that single-pass fitting with conservative culling outperforms progressive multi-pass approaches at a fixed splat budget for 8 of 12~datasets, while 2-pass progressive fitting yields +0.9 to +1.7~dB gains for clean, well-structured volumes---and is consistently 2--9$\times$ faster (Supplementary Fig.~4; Supplementary Document~3).
...
%% ========== TABLE ==========
\input{figs/table/table_summary.tex}
...
Comparison at a fixed 32K-splat budget across 12~datasets. Single-pass wins for 8/12 datasets (mostly confocal); 2-pass progressive gains +0.9 to +1.7~dB for 4 datasets with clean, well-structured signals, and is 2--9$\times$ faster across all datasets.
...
Four datasets with clean, well-structured signals (HeLa, Tribolium, mouse heart) gain +0.9 to +1.7~dB from 2-pass fitting; eight noisier confocal datasets favour single-pass.
\textbf{b,} Wall-clock speedup of 2-pass over 1-pass fitting.
Progressive fitting is 2--9$\times$ faster for all 12~datasets because fitting smaller batches of splats is sub-linearly cheaper per iteration.}
\end{figure*}
