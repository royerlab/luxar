...
\title{\bfseries Gaussian Splat Count vs.\ Reconstruction Quality:\\
       Rate-Distortion Analysis with Noise2Self Model Selection}
...
based on blind-spot cross-validation inspired by Noise2Self
...
\begin{enumerate}
  ...
  \item \textbf{Noise2Self model selection} (\cref{sec:noise2self}):
...
\midrule
opencell\_map4\_ch0 & Spinning-disk & Nuclei (Hoechst) & $51\times600\times600$ & 18.4\,M \\
...
% ══════════════════════════════════════════════════════════════════
\section{Noise2Self Model Selection}
...
Inspired by the Noise2Self framework \citep{batson2019noise2self}, we
...
\begin{remark}[Connection to the Noise2Self theorem]
...
volume $\tilde{Y}$. The key insight from the Noise2Self framework is
...
The Noise2Self analysis reveals that \textbf{10 of 12 datasets exhibit
...
\begin{table}[H]
\centering
\caption{Noise2Self model selection summary.}
...
\paragraph{4. The N2S held-out peak is a principled stopping criterion.}
...
The Noise2Self framework requires pixel-independent noise. Preprocessing
...
three imaging modalities. The Noise2Self-inspired blind-spot cross-validation
...
\newcommand{\datasetfigs}[2]{%
  ...
  \begin{figure}[H]
    ...
    \caption{Noise2Self analysis for #2.}
  \end{figure}
}

\datasetfigs{opencell_map4_ch0}{OpenCell MAP4 --- Hoechst (Nuclei)}
\datasetfigs{opencell_map4_ch1}{OpenCell MAP4 --- GFP (Microtubules)}
\datasetfigs{opencell_lmnb1_ch0}{OpenCell LMNB1 --- Hoechst (Nuclei)}
\datasetfigs{opencell_lmnb1_ch1}{OpenCell LMNB1 --- GFP (Nuclear Lamina)}
