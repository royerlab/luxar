...
Gaussians (``splats''), each parameterised by a centre~$\bmu$, an
amplitude~$a$, and a covariance matrix~$\Sig$
\citep{kerbl2023}.  For large scientific volumes, $N$ can reach
millions, making simplification essential for interactive
visualisation and efficient storage.
...
Several divergences could measure approximation quality
(\cref{tab:divergences}).  The $L^2$ (integrated squared error)
framework is uniquely suited because \emph{the integral of a product
of two Gaussians has a closed-form expression}.  Every term in the
...
\begin{equation}\label{eq:splat_def}
  ...
  \Sig \in \R^{D\times D} \text{ s.p.d.}
\end{equation}
The covariance $\Sig$ is stored via its lower-triangular Cholesky
factor $\Lchol$ satisfying $\Sig = \Lchol\Lchol^\top$.  We denote
the precision matrix $\Prec = \Sig^{-1}$.  In the Luxar implementation,
...
The Gaussian family forms an exponential family with sufficient
statistics $\bx$ and $\bx\bx^\top$ \citep{bishop2006}.
A classical result from information geometry states that the minimiser
of the forward KL divergence
$\mathrm{KL}\!\left(\frac{f}{\int f}\,\middle\|\,
  \frac{g}{\int g}\right)$
over any member of an exponential family matches the sufficient
statistics of the target---i.e., it matches the mean and
covariance of the \emph{normalised} mixture~\citep{bishop2006, west1993}.
...
from~\eqref{eq:mm_amp}, we can fix the moment-matched shape parameters
...
Normalising~\eqref{eq:E_star} by the mixture's total energy gives a
dimensionless score (\cref{fig:angle_diagram}):
