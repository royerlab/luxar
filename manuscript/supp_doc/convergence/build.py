#!/usr/bin/env python3
"""Build the convergence supplementary document.

Concatenates per-dataset convergence PDFs into a single document.
...
ANALYSIS_RESULTS = HERE.parent.parent / "analysis" / "convergence" / "results"
ANALYSIS_DIR = HERE.parent.parent / "analysis" / "convergence"
...
def main():
    print("Building: convergence_all.pdf")

    pdfs = []
    for ds in DATASETS:
        p = ANALYSIS_RESULTS / ds / "fig_convergence.pdf"
        if p.exists():
            pdfs.append(p)

    if not pdfs:
        print("  No PDFs found. Run the convergence analysis first.")
        return

    out = HERE / "convergence_all.pdf"
