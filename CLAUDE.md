...
hatch run mypy packages/luxar/src/luxar/ scripts/ci_queue_scan.py scripts/check_cadence_liveness.py  # Type check (host only)
make type-check-python            # Type check for the host, Darwin, and Python 3.14
```
...
# The reference target is the reflections EXAMPLE store (`make run-examples` builds it):
luxar env bake datasets/examples/mesh_reflections_example.luxar.zarr
...
# ONE uniform grid, whichever entry point builds it (#2838): a trailing tile
# whose unique coverage is thinner than the overlap is FOLDED into its
# predecessor, by `fit --tiling uniform` (sequential and `-j N`), `fit --tile
...
luxar gsplat batch-fit submit data.zarr.zip output/ -p gpu --parallel         # Packed workers; scales per-task CPU/RAM and pins worker resources
