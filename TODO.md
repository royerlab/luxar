...
- **R17 [LAUNCH] — Retire git-LFS for heavy processed datasets → Zenodo
  ...
  - **Reusable mechanism already in-tree:** `demos/demo_cosmicflows_laniakea.py`
    ...
    `luxar.demos` helper that pulls a named dataset from a Zenodo
  ...
  - **Step 1 landed.** `packages/luxar/src/luxar/demos/data_manifest.json`
    ...
    `luxar.demos` (`ensure_dataset`, `load_dataset_gsplats`) +
  ...
  - **Fetch infra — BUILT (reversible piece, 2026-07-22; branch `r17-data-fetch-helper`).**
    ...
    - `luxar.demos.ensure_dataset(name, variant=…)` — resolves files
      **cache → in-repo LFS → Zenodo**, dormant on the Zenodo leg until the
      record is `published` (or carries an explicit `base_url`), so demos keep
      working on the in-repo fallback today. Raises `LocalComputeDataset` for
      `local-compute`/`regenerate` datasets.
    - **Size variants:** `h2afva` ships as two variants in one record — `51tp`
      (2.1 GB, **default** the demo fetches) and `253tp` (11.4 GB, opt-in via
      `variant="253tp"`). Avoids an 11.4 GB first-run download over Zenodo's
  ...
  - **Status check 2026-08-14 (supersedes the counts above): all three
    depositions now EXIST as unsubmitted drafts** — `cc-by` 21912280, `cc-by-sa`
    21912282, `h2afva` 21912284 — and the manifest records their ids and reserved
    *version* DOIs (the concept DOI is only minted at publication, so it is not
    knowable yet). Each carries `published: false`, and `zenodo_file_url` builds
    no derived URL while that is false, so the leg is still dormant and every
    demo still runs off in-repo LFS. Uploaded and now pinned by sha256:
    `gsplats_4d_neuromast_2ch` (both channels, 133 MB) and h2afva's **253tp**
    (11.4 GB, not the 16 GB estimated above). Corrections to the notes above:
    51tp measures **2.1 GB**, and it is a temporal subsample (every 5th frame)
    from an INDEPENDENT fit, *not* a strict subset of the 253tp — per-timepoint
