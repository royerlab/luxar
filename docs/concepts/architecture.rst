...
* ❌ **Mesh authoring / CAD** - Luxar *renders* triangle meshes (``add_mesh``, ``luxar mesh import``), but it does not edit them, and a mesh has no spatial index (see ``docs/specs/MESH_NODE_SPEC.md`` §9), so one level always loads whole. Spatial partitioning **is** available (``add_mesh(partition=…)``), which splits a large surface into frustum-cullable parts — but each part still loads whole
