...
| **6** | Fixture + E2E spec + demo + docs + CHANGELOG | Shippable. Note the one honest gap carried in from Phases 4–5: the parity harness drives `WebGPURenderer({ forceWebGL: true })`, so the real-WGSL path is unexercised and both backend divergences above are safe by CONSTRUCTION rather than by test. A real-WebGPU pass belongs with this phase's E2E work |
