...
`data/scene-loader/process/data-processor-mesh.ts` logs one `info` line per node and hidden
dimension, naming the dimension and its unit. That line is also the evidence gate on §9's deferral
of exact clipping.
