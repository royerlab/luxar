...
5. On Linux: builds `linux-<host-arch>` (requires `libwebkit2gtk-4.0-dev` + `pkg-config` — the pinned `webview_go` declares `#cgo pkg-config: gtk+-3.0 webkit2gtk-4.0`, which is why CI builds the launcher on ubuntu-22.04; 24.04 ships only the 4.1 package)
