...
5. Sphinx builds the HTML documentation with `-E -a -W --keep-going`, forcing a
...
The most common way to trip it is adding a file under `docs/`: Sphinx warns that
a document is not included in any toctree, and `-W` makes that fatal. Add the
new page to a toctree in `docs/index.rst`, or, if it is not meant to be
published, to `exclude_patterns` in `docs/conf.py`.
