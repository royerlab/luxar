# Luxar Performance Optimization Specification (REVISED)

**Version**: 2.0.0 (Complete Revision)
**Date**: 2025-12-22
**Status**: Ready for Implementation
**Revision Notes**: Addresses all critical issues from v1.0 review
...
- **GSplats**: centers, amplitudes, cholesky_factors, colors, sharpness
...
GSplatsSpatialIndexLoader
  ├─ Data: centers, amplitudes, cholesky_factors, colors, sharpness
  ├─ Loading: Single-phase (query chunks → load data)
...
### 1.3 Data Flow Example: Lines (Two-Phase Loading)
...
User changes dimension slider (e.g., Time: 10 → 11)
  ...
  │     │
  │     └─► LinesSpatialIndexLoader.loadLines(viewState)
  │           │
  │           ├─ Acquire persistent LinesDataAccumulator (no alloc if capacity OK)
  │           │
  │           ├─ Post to Worker: { task: 'lines-spatial-query', viewState }
  │           │   (Transferable: segmentChunkBounds, vertexChunkBounds)
  │           │
  │           ▼
  ├─► Worker Thread: LinesQueryTask.execute()
  │     │
  │     ├─► PHASE 1: Query segment chunks
  │     │     - WASM: query_segment_chunks_for_view()
  │     │     - Returns: matchingSegmentChunks = [12, 45, 67, ...]
  │     │
  │     ├─► PHASE 2: Load segments and decode
  │     │     - For each segment chunk:
  │     │       - Fetch encoded chunk from zarr (may hit L1/L2 cache)
  │     │       - ArrayDecoder.decode() (handles Blosc/LUT/quantization/etc)
  │     │     - Result: segmentIndices array (global vertex indices)
  │     │
  │     ├─► PHASE 3: Derive vertex ranges
  │     │     - Analyze segmentIndices to find referenced vertices
  │     │     - Determine which vertex chunks to load
  │     │     - Result: vertexRanges = [{start:1000, end:2000}, ...]
  │     │
  │     ├─► PHASE 4: Load vertices and decode
  │     │     - For each vertex range:
  │     │       - Fetch and decode with ArrayDecoder
  │     │     - Result: vertices (positions, colors, etc.)
  │     │
  │     ├─► PHASE 5: Remap indices
  │     │     - Convert global indices → local indices (0-based)
  │     │     - Result: localIndices
  │     │
  │     ├─► PHASE 6: Clip segments at slice boundaries
  │     │     - WASM: clip_segments_nd()
  │     │     - Interpolate colors/widths for clipped endpoints
  │     │     - Result: clippedSegments
  │     │
  │     └─ Post back: { linesData: {...}, metadata: {...} }
  │         (Transferable: vertices, indices, colors, widths arrays)
  │           │
  │           ▼
  ├─► Main Thread: LinesSpatialIndexLoader.handleWorkerResult()
  │     │
  │     ├─ Receive Transferable (zero-copy)
  │     ├─ Store in persistent LinesDataAccumulator
  │     │
  │     └─► SceneManager.updateLines(nodeId, linesData)
  │           │
  │           ├─► GPU Buffer Pool.acquireLinesGeometry(nodeId, segmentCount, vertexCount)
  │           │     - Reuse existing geometry if capacity OK
  │           │     - Partial update: buffer.addUpdateRange(0, count)
  │           │     - GPU uploads only changed region
  │           │
  │           └─► Render with updated THREE.Line (no disposal/reallocation)
  │
...
import type { ArrayMetadata } from './array-decoder';
...
export interface DataAccumulator<TData> {
  ...
  /**
   * Get data view with specified count
   * WARNING: Returned arrays are views into persistent buffers
   */
  getData(count: number): TData;

  /**
   * Fill accumulator at offset
   */
  fill(offset: number, data: Partial<TData>): void;
...
export class PointsDataAccumulator implements DataAccumulator<PointsData> {
  ...
  /**
   * Get PointsData with metadata (CORRECT interface!)
   */
...
/**
 * Lines data accumulator (handles vertices + indices separately)
 */
export class LinesDataAccumulator implements DataAccumulator<LoadedLinesData> {
  // Separate buffers for vertices and segment data
  private vertexPositionBuffer: Float32Array;
  private vertexColorBuffer: Uint8Array;
  private segmentIndexBuffer: Uint32Array;
  private segmentWidthBuffer: Float32Array;
  private segmentSharpnessBuffer: Float32Array;
  ...
  constructor(
    ...
    this.vertexPositionBuffer = new Float32Array(initialVertexCapacity * 3);
    this.vertexColorBuffer = new Uint8Array(initialVertexCapacity * 4);
    this.segmentIndexBuffer = new Uint32Array(initialSegmentCapacity * 2);
    this.segmentWidthBuffer = new Float32Array(initialSegmentCapacity);
    this.segmentSharpnessBuffer = new Float32Array(initialSegmentCapacity);
  ...
  ensureCapacity(needed: number): boolean {
    ...
    if (neededVertices > this.vertexCapacity) {
      ...
      const newVertexPos = new Float32Array(newVertexCap * 3);
      const newVertexColor = new Uint8Array(newVertexCap * 4);

      newVertexPos.set(this.vertexPositionBuffer);
      newVertexColor.set(this.vertexColorBuffer);

      this.vertexPositionBuffer = newVertexPos;
      this.vertexColorBuffer = newVertexColor;
    ...
    if (neededSegments > this.segmentCapacity) {
      ...
      const newSegmentIdx = new Uint32Array(newSegmentCap * 2);
      const newSegmentWidth = new Float32Array(newSegmentCap);
      const newSegmentSharpness = new Float32Array(newSegmentCap);

      newSegmentIdx.set(this.segmentIndexBuffer);
      newSegmentWidth.set(this.segmentWidthBuffer);
      newSegmentSharpness.set(this.segmentSharpnessBuffer);

      this.segmentIndexBuffer = newSegmentIdx;
      this.segmentWidthBuffer = newSegmentWidth;
      this.segmentSharpnessBuffer = newSegmentSharpness;
  ...
  getData(count: number): LoadedLinesData {
    // For lines, count = number of segments
    // Estimate vertex count (will be set properly by fill())
    const estimatedVertices = Math.ceil(count * 1.5);

    return {
      vertices: {
        positions: this.vertexPositionBuffer.subarray(0, estimatedVertices * 3),
        colors: this.vertexColorBuffer.subarray(0, estimatedVertices * 4),
      },
      segments: {
        indices: this.segmentIndexBuffer.subarray(0, count * 2),
        widths: this.segmentWidthBuffer.subarray(0, count),
        sharpness: this.segmentSharpnessBuffer.subarray(0, count),
      },
  ...
  fill(offset: number, data: Partial<LoadedLinesData>): void {
    if (data.vertices?.positions) {
      const vertexOffset = Math.floor(offset * 1.5); // Estimate
      this.vertexPositionBuffer.set(data.vertices.positions, vertexOffset * 3);
    }
    if (data.vertices?.colors) {
      const vertexOffset = Math.floor(offset * 1.5);
      this.vertexColorBuffer.set(data.vertices.colors, vertexOffset * 4);
    }
    if (data.segments?.indices) {
      this.segmentIndexBuffer.set(data.segments.indices, offset * 2);
    }
    if (data.segments?.widths) {
      this.segmentWidthBuffer.set(data.segments.widths, offset);
    }
    if (data.segments?.sharpness) {
      this.segmentSharpnessBuffer.set(data.segments.sharpness, offset);
    }
  }
  ...
  getStats(): AccumulatorStats {
    return {
      ...
      growthEvents: this.totalGrowths,
      memoryMB:
        (this.vertexCapacity * 16 + // vertices (pos+color)
          this.segmentCapacity * 16) / // segments (idx+width+sharp)
        1024 /
        1024,
    };
  }
  ...
  dispose(): void {
    this.vertexPositionBuffer = new Float32Array(0);
    this.vertexColorBuffer = new Uint8Array(0);
    this.segmentIndexBuffer = new Uint32Array(0);
    this.segmentWidthBuffer = new Float32Array(0);
    this.segmentSharpnessBuffer = new Float32Array(0);
...
export class GSplatsDataAccumulator implements DataAccumulator<LoadedGSplatsData> {
  ...
  private choleskyBuffer: Float32Array; // Size depends on ndim: ndim*(ndim+1)/2
  private colorBuffer: Uint8Array;
  private sharpnessBuffer: Float32Array;
  ...
  getData(count: number): LoadedGSplatsData {
    return {
      ...
      cholesky_factors: this.choleskyBuffer.subarray(0, count * this.choleskySize),
      colors: this.colorBuffer.subarray(0, count * 4),
      sharpness: this.sharpnessBuffer.subarray(0, count),
      splatCount: count,
      ndim: this.ndim,
  ...
  fill(offset: number, data: Partial<LoadedGSplatsData>): void {
    ...
    if (data.cholesky_factors) {
      this.choleskyBuffer.set(data.cholesky_factors, offset * this.choleskySize);
    }
    if (data.colors) {
      this.colorBuffer.set(data.colors, offset * 4);
    }
...
export class PointSpatialIndexLoader implements DataLoader {
  ...
  /**
   * CORRECT METHOD NAME (matches interface)
   */
...
**Similar updates for `LinesSpatialIndexLoader` and `GSplatsSpatialIndexLoader`**.
...
async function decodeArrayChunk(params: {
  ...
  const { zarrArrayPath, chunkIndex, attrs, zarrStoreUrl } = params;

  // Open zarr array
  // Note: This requires zarr store access from worker
  // May need to pass encoded chunk data directly instead
  const zarrStore = new zarr.FetchStore(zarrStoreUrl);
  const location = zarr.root(zarrStore).resolve(zarrArrayPath);
  const zarrArray = await zarr.open(location, { kind: 'array' });

  // Decode using ArrayDecoder (handles ALL encoding types)
  const decoded = await arrayDecoder!.decode(
    zarrArray,
    attrs,
    undefined, // expectedElements
    zarr.root(zarrStore) // For array_ref resolution
  );
...
/**
 * Task 6: Interleave and filter attributes (generic pattern)
 */
async function interleaveAttributes(params: {
  attributes: { [key: string]: TypedArray };
  visibilityMask: Uint8Array;
  numElements: number;
  visibleCount: number;
}): Promise<{ [key: string]: TypedArray }> {
  // For each attribute, filter by visibility mask
  const result: { [key: string]: TypedArray } = {};

  for (const [name, array] of Object.entries(params.attributes)) {
    const elementSize = array.length / params.numElements;
    const ArrayType = array.constructor as any;
    const filtered = new ArrayType(params.visibleCount * elementSize);

    let outIdx = 0;
    for (let i = 0; i < params.numElements; i++) {
      if (params.visibilityMask[i]) {
        for (let j = 0; j < elementSize; j++) {
          filtered[outIdx * elementSize + j] = array[i * elementSize + j];
        }
        outIdx++;
      }
    }
...
- Decoding: 20-40ms (ArrayDecoder on main thread)
- Visibility: 15-30ms
- Total: 40-80ms = **12-25 FPS**

**After worker offload:**
- Main thread: ~1ms (message passing only)
- Worker thread: 30-60ms (runs in parallel, with WASM speedup)
...
export class GPUBufferPool {
  ...
  acquirePointsGeometry(nodeId: string, pointCount: number): THREE.BufferGeometry {
    ...
    // Try to find in pool
    for (const [bucket, buffers] of this.pointBuffers.entries()) {
  ...
  private createLinesGeometry(segmentCapacity: number, vertexCapacity: number): THREE.BufferGeometry {
    ...
    geometry.setAttribute('color',
      new THREE.Uint8BufferAttribute(new Uint8Array(vertexCapacity * 4), 4, true));
    ...
    // Per-segment attributes (need special handling - maybe instanced?)
  ...
  /**
   * Update geometry with new data (partial updates)
   */
  updatePointsGeometry(
    ...
    // Similar for color, radius, sharpness...

    geometry.setDrawRange(0, count);
  }

  // Similar updateLinesGeometry(), updateGSplatsGeometry()...
...
export class SceneManager {
  ...
  updateGSplats(nodeId: string, gsplatsData: LoadedGSplatsData): void {
    ...
    gpuBufferPool.updateGSplatsGeometry(geometry, gsplatsData);
...
**Test Lines Accumulator (Two-Phase):**
```typescript
describe('LinesDataAccumulator', () => {
  it('should handle vertices and segments separately', () => {
    const acc = new LinesDataAccumulator(1000, 500);

    acc.fill(0, {
      vertices: {
        positions: new Float32Array([0, 0, 0, 1, 1, 1]), // 2 vertices
        colors: new Uint8Array([255, 0, 0, 255, 0, 255, 0, 255]),
      },
      segments: {
        indices: new Uint32Array([0, 1]), // 1 segment
...
**Test Worker with ArrayDecoder:**
```typescript
describe('DataWorker with ArrayDecoder', () => {
  ...
  it('should decode quantized arrays', async () => {
    // Mock quantized array
    ...
    // Create mock zarr array with quantized data
    // ... (test implementation)

    const decoded = await worker.decodeArrayChunk({
      zarrArrayPath: 'positions',
      chunkIndex: [0],
      attrs,
      zarrStoreUrl: 'mock://store',
    });

    expect(decoded).toBeInstanceOf(Float32Array);
    expect(decoded.length).toBe(300); // 100 * 3
  });

  it('should fail without WASM support', async () => {
...
**Test GPU Buffer Pool (All Types):**
```typescript
describe('GPUBufferPool', () => {
  ...
  it('should reuse Points geometry', () => {
    const geom1 = pool.acquirePointsGeometry('node1', 1000);
    pool.release('node1');
    const geom2 = pool.acquirePointsGeometry('node1', 900);
    expect(geom1).toBe(geom2); // Same object reused
  });

  it('should handle Lines geometry', () => {
    const geom = pool.acquireLinesGeometry('node1', 500, 1000);
    expect(geom.attributes.position).toBeDefined();
    expect(geom.index).toBeDefined(); // Lines need index buffer
  });

  it('should handle GSplats geometry', () => {
    const geom = pool.acquireGSplatsGeometry('node1', 1000, 3);
    expect(geom.attributes.center).toBeDefined();
    expect(geom.attributes.cholesky).toBeDefined();
  });
});
```
...
**End-to-End Load Performance (All Types):**
```typescript
describe('Load Performance (Points, Lines, GSplats)', () => {
  it('should load 100K points in <50ms', async () => {
    ...
    const pointsData = await loader.loadPoints(mockViewState);
    const loadTime = performance.now() - startTime;

    expect(pointsData.metadata.loadedPoints).toBeGreaterThan(0);
    expect(loadTime).toBeLessThan(50);
  });

  it('should load 50K lines (two-phase) in <80ms', async () => {
    // Test Lines loader specifically
    // ... (two-phase loading test)
  });

  it('should load 100K gsplats in <60ms', async () => {
    // Test GSplats loader
    // ...
  });

  it('should sustain 60fps during navigation', async () => {
    // Simulate slice navigation for all three types
    // Measure frame time
...
### 8.1 Target Specifications (All Types)
...
**Deliverables:**
- `data-accumulator.ts` with all three types
- Integration into all three loaders
...
### 10.1 No Backwards Compatibility (Clarified)

**Policy: Option C - No fallbacks, no old formats**
...
- ✅ Fixes all interface mismatches (PointsData, loadPoints/updateView)
- ✅ Supports ALL three types equally (Points, Lines, GSplats)
- ✅ Integrates ArrayDecoder properly
- ✅ Clarifies backwards compatibility (Option C - no fallbacks, no old formats)
- ✅ Explains relationship to existing cache (complementary, not conflicting)
- ✅ Addresses Lines two-phase loading
- ✅ Provides complete code examples with correct types
- ✅ Defines proper config integration
- ✅ Covers all three geometry types in GPU Buffer Pool

**Status: READY FOR IMPLEMENTATION**

---

**Document Version**: 2.0.0 (Complete Revision)
**Date**: 2025-12-22
