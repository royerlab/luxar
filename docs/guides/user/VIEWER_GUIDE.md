...
| `cacheBudgetMB` | number | Total in-memory cache pool (L0 + L1 + S-cache) in MB, for environments without `performance.memory` (Safari, WKWebView). Also tightens the auto GPU-geometry/LOD residency budget so constrained-device behavior can be reproduced on roomy hardware. |
