...
| `depthShards` | number | Cross-node depth ordering: split each order-dependent node that OVERLAPS another into `N` contiguous depth ranges, so ranges of different nodes interleave instead of one node drawing entirely before the other. Off by default; `0` disables the subsystem outright (pins the draw order, for deterministic runs). Costs measured GPU time on scenes that actually overlap — see `docs/guides/specs/CROSS_NODE_DEPTH_ORDERING_SPEC.md`. |
