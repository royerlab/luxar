...
To follow along without installing anything, open a scene on the live demo gallery at [demos.luxarviewer.dev](https://demos.luxarviewer.dev) — every control described below is present there. The same viewer is deployed standalone at [luxarviewer.dev](https://luxarviewer.dev), which takes any reachable archive as `?src=<url>`.
