...
> **Status**: Spike + Phase 0 SHIPPED (PR #511) — spike verdict PASS on all 3 shader surfaces; `normal` mode emits premultiplied coverage alpha via `LUXAR_NORMAL_PREMULT` + `getGSplatNormalBlendingState()`. Phases 1–3 remain PLANNED. (Rev 3 after two adversarial re-checks; risk register §10 — including post-ship fix notes — and changelogs §11.)
