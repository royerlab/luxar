...
> **Status**: PLANNED — no implementation yet. (Rev 3 after two adversarial re-checks; risk register §10, changelogs §11.)
...
- `normal` mode (`rendering/blending-state.ts:178-189`) is SrcAlpha/OneMinusSrcAlpha alpha-over — **non-commutative**, therefore wrong in arbitrary order. `additive`/`luminous`/`max` are commutative and unaffected.
- **GSplats cannot alpha-blend at all today**: the fragment shader premultiplies intensity into RGB and emits `alpha = 1.0` (`materials/gsplat/shader-glsl.ts:404-417`), with an in-code note that proper transparent normal blending "requires premultiplied alpha with ONE / ONE_MINUS_SRC_ALPHA" — deferred until needed. That fix is Phase 0 here.
...
- Vertex texture fetch is **already in production**: the colormap LUT is sampled in the gsplat *vertex* shader (`shader-glsl.ts:312`, `#ifdef USE_COLORMAP`).
- Pick nodes **share the visual mesh's geometry** and pick materials are **already per-node** (`node-factory/create-gsplats-node.ts:68-77`). Pick shaders read the same five instanced attributes (`picking/gsplat/shaders.ts:35-39`, `pick.tsl.ts:85-89`). Post-commit material sync has an established home: `rendering/material-sync-helpers.ts` (points radiusScale precedent, pushes to render + pick materials via `userData.pickNode`).
...
| Texture ownership | The **pool entry** owns the texture: `PooledBuffer` gains `splatTexture`; commit rebinds it via a new `syncGSplatMaterialWithGeometry` in `material-sync-helpers.ts` (render + pick material, same shape as the points radiusScale sync) | Texture lifetime = geometry lifetime (reuse, growth, LRU + byte-budget eviction, `dispose()`); rebind rides the established post-commit sync seam |
...
1. **Fragment shaders** (`materials/gsplat/shader-glsl.ts` + `shader-tsl.ts`): under a new `LUXAR_NORMAL_PREMULT` define (GLSL) / factory flag (TSL), emit premultiplied coverage alpha:
   ...
   ```
   Default (no define) output is byte-identical to today — additive/luminous/max keep the exact `vec4(rgb, 1.0)` contract (the GLSL additive path's alpha-MaxEquation HalfFloat16 overflow guard depends on it). Note the deliberate HDR asymmetry: RGB carries unclamped `intensity × uOpacity`, alpha carries the clamped coverage — an emitter-with-occlusion model, correct for `1-α` destination attenuation.
...
1. **Pool adapter** (`rendering/gpu-buffer-pool/gsplats-adapter.ts`):
   ...
   - `PooledBuffer` (in `pool-stats.ts`) gains `splatTexture: THREE.DataTexture`. Growth (`growGSplatsGeometry`) reallocates texture + ordering attribute at `chooseCapacity` steps with old-data carry-forward, calls `invalidateCachedByteSize`; the existing `_lastAcquireRebuilt` → `invalidateRenderObjectFor(mesh)` contract (`commit-gsplats-geometry.ts:93-96`) still covers the ordering attribute rebuild, and the texture swap is covered by the material rebind (below).
   ...
   - Non-pool fallback (`gsplat-geometry.ts::createInstancedGSplatsMesh` / `updateInstancedGSplatsMesh`) gets the same treatment with a mesh-owned texture (note: the pre-existing broken-non-pool-fallback finding from the 2026-07-12 engine review must be fixed or explicitly WONTFIXed in the same PR rather than ported broken).
...
- Unit: texel address math incl. non-4096 widths; fused writer round-trip (write staged arrays → read back texel layout); pool growth carry-forward; byte accounting; material-disposal leak check.
...
- Perf gate: `Update Buffers` profiler stage (update-profiler session in `commit-gsplats-geometry.ts`) not regressed on the timelapse-nav benchmark; VRAM within the +31% envelope on demo scenes.
...
- **Points/Lines symmetry**: the three-geometry symmetry policy is deliberately broken *temporarily* — gsplats are the only type with the alpha=1 blocker and the 10M-scale requirement. The pool-adapter/texture pattern and `aSortedIndex` interface are designed to be liftable to `points-adapter.ts` (8-9 floats → 2-3 texels) verbatim. Lines need a midpoint-key approximation (segments have extent) and are lowest priority.
...
1. **Three r184 half-supported vertex paths — the twice-burned pattern (highest risk).** This repo has already been bitten twice by r184 vertex-pipeline gaps: `GLSLNodeBuilder` hardcoding `gl_PointSize = 1.0` (forced the Points→instanced-quad migration) and `gpuType: HalfFloatType` silently ignored on interleaved attributes (the `dd7c4478` Float16 revert that shipped a regression). Phase 1 bets on two more under-exercised paths: a **uint instanced attribute** and **vertex-stage `textureLoad`**, each through *both* the raw-GLSL ShaderMaterial path and the TSL node-builder (including WebGPURenderer's WebGL2-fallback GLSL emission). Raw WebGL2 support is verified (§1); what is NOT verified is what three's node builders emit. **Mitigation — mandatory spike before Phase 1 is scoped**: a 1-day throwaway probe rendering one quad-instanced mesh with a uint attribute + vertex texelFetch on all three surfaces (WebGL, WebGPU-native, `webgpu-force-webgl`). Fallback if uint attributes are broken in the node builder: a float attribute carries exact integers to 2^24 = 16.7M — exactly our per-node cap, so it works but eliminates headroom.
   ...
   >
   > **DEBT-REMEDIATION UPDATE (2026-07-13).** The vacuous-parity issue this spike's methodology flagged is FIXED: harness meshes now use production assembly, every sprite parity test carries a non-vacuousness guard + a footprint-invariant per-covered-pixel metric, and two REAL production TSL bugs the vacuousness had hidden were found and fixed (branch-scoped materialization of shared eigen values in `shader-tsl.ts`/`pick.tsl.ts` — quads degenerate for near-diagonal Σ2D; both vertex stages now trace inside `Fn()` with explicit `.toVar()` statements — the TSL house rule going forward). Post-fix, GLSL↔TSL parity is PIXEL-EXACT (per-covered diff 0.00) on every variant. **Still open (tracked follow-up): production gsplats under `?renderer=webgpu` (WebGL2-fallback backend) render as self-intersecting "bowtie" quads** — the harness path (production mesh creator, same shaders) is pixel-perfect and production WebGL (pool geometry) is perfect, so the defect sits in the GPU-buffer-pool geometry × WebGPU-fallback binding interaction (suspects: capacity>count interleaved buffer binding, index/corner buffer handling in the fallback backend, or the RenderObject invalidation contract). Evidence: bowtie screenshot of test_gsplats fixture, `backend.isWebGLBackend=true`, correct uniform values probed live. Phase 1's WebGPU gates must include this being fixed or explicitly triaged.
