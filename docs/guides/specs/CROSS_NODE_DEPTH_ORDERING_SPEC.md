...
> **Status**: DESIGN + Phase S COMPLETE; Phases 0–3 not yet implemented. Work lands on the draft holding PR #2407 (branch `feat/cross-node-depth-shards`). Two questions gated the design and **both have been answered by measurement, not estimate**: what splitting one instanced draw into S costs (§7), and which attribute layout can express a shard draw on every renderer backend (§7.1). The design below is written against those results.
