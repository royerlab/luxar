...
def diagnose_gsplat_rendering():
    ...
    fx = resolution[0] / (2 * np.tan(fov_rad / 2))  # Same formula (aspect ratio handled)
    ...
    print(f"   Sum = amplitude × σ_z × √(2π) = {amplitude} × {sigma_z} × {np.sqrt(2*np.pi):.4f}")
    ...
    if abs(ratio - 1.0) > 0.01:
        ...
        print(f"   - For this axis-aligned case, σ_ray should equal σ_z")

        if abs(sigma_ray - sigma_z) > 0.01:
            print(f"\n   ❌ σ_ray ≠ σ_z")
            print(f"   This is because ray_dir is not [0,0,1] but {ray_dir}")
            print(f"   σ_ray = sqrt(ray @ Σ @ ray) depends on all elements of Σ")
    else:
        print(f"\n✓ Intensities match!")
    ...
    print(f"This IS [0, 0, -1], so σ_ray SHOULD equal σ_z")
...
def diagnose_perspective_distortion():
    ...
    print(f"{'Position':<20} {'σ_x (px)':<12} {'σ_y (px)':<12} {'Ratio':<10} {'Area Ratio':<12}")
    print("-" * 70)
    ...
    for name, pos in test_positions:
        ...
        print(f"{name:<20} {sigma_x:<12.2f} {sigma_y:<12.2f} {ratio:<10.2f} {area_ratio:<12.2f}")
...
def diagnose_actual_gsplat_data(path: str):
    ...
    try:
        ...
        def find_gsplats(group, grp_path=""):
            ...
            has_gsplats_arrays = 'centers' in group.keys() and 'cholesky_factors' in group.keys()
    ...
    if n_splats > 1 and n_splats < 100000:
        ...
        distances, _ = tree.query(data.centers, k=2)  # k=2 to get nearest neighbor (excluding self)
    ...
    for d in range(ndim):
        ...
        print(f"    L[{d},{d}]: min={min_val:.4f}, max={max_val:.4f}, mean={mean_val:.4f}")
    ...
    for d in range(ndim):
        ...
        if valid.sum() > 0:
            ...
            print(f"    σ_{d}: min={min_val:.4f}, max={max_val:.4f}, mean={mean_val:.4f}")
        else:
    ...
    if n_splats > 1 and n_splats < 100000:
        ...
        if np.isnan(coverage_ratio):
            ...
            print(f"     This could cause visible gaps in rendering.")
            print(f"     Splats would need σ ≈ {avg_spacing / 2:.4f} for smooth coverage.")
    ...
    if np.allclose(data.sharpnesses, 2.0, atol=0.01):
        print(f"  ✓ All sharpnesses ≈ 2.0 (standard Gaussian)")
    else:
        print(f"  Note: Sharpness varies from standard Gaussian (s=2)")
