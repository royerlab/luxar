...
6. **Never tighter than the footprint**: A stored `chunk_bounds` interval contains the chunk's geometric footprint — as AUTHORED — at ANY coordinate magnitude. All four builders (points, gsplats, segments, vertices) accumulate in float64 and narrow to the float32 store with outward rounding — see *Float32 Outward Store* above. Coordinates are themselves stored as per-axis uint16 fixed point by default, so a DECODED coordinate could sit up to half a quantum (`extent/131070`) outside a bound derived from the authored one on a non-gridded axis; both geometries close that gap, differently. GSplats escalate an offending centers axis to float32 (`_compiler/gsplat_assembly.py`), while points and lines keep the uint16 encoding and take an extra per-axis `coord_slack` pad, which the compiler fills from the encoder's own `ArrayEncoder.coordinate_round_trip_slack` (zero on an axis the encoder stores exactly — gridded, constant or float32-fallback — and zero for the whole array when the write will genuinely store a LUT). LUT eligibility alone is NOT exemption: the predicate takes an `allow_lut` mirroring `ArrayEncoder.encode`'s, and the lines glue passes `allow_lut=False` because the lines writer blocks LUT on `vertices` (the spatial-index loader reads it raw), so a LUT-eligible lines node is quantised and still gets the pad. The points glue leaves it at the default, because `write_positions` does not block LUT and a LUT-eligible points node really does store `lut_uint8`. A DIRECT caller of a bound builder that omits `coord_slack` still gets authored-coordinate bounds. Swept over magnitudes `{0, 1e-6, 1, 100, 2**23, 2**24, 2**24+1, 2**25, 1e9, 1e15, 1e20, 1e30, 3e38}` × sign × pad by `io/tests/test_ordering_properties.py::test_chunk_bounds_contain_the_footprint_at_every_magnitude`.
