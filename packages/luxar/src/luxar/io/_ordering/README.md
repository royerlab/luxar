...
6. **Never tighter than the footprint**: A stored `chunk_bounds` interval contains the chunk's geometric footprint at ANY coordinate magnitude. All four builders accumulate in float64 and narrow to the float32 store with outward rounding. Points and lines additionally take the encoder's per-axis `coord_slack` on every dimension, and their radius/width builders take one `scalar_slack` on spatial dimensions only. The compiler fills those from `ArrayEncoder.coordinate_round_trip_slack` and `ArrayEncoder.positive_scalar_round_trip_slack`, so the bound contains the DECODED coordinate and DECODED footprint scalar rather than only the authored values. The slack query must mirror the write's `allow_lut` value: points positions allow exact LUT storage, while lines vertices forbid it because the spatial-index loader reads raw chunks. Direct callers may omit either pad to retain authored-value bounds. GSplats retain their separate float32-escalation rail; its known non-gridded barrier-axis limit is documented on `compute_chunk_bounds_gsplats`. Swept over magnitudes `{0, 1e-6, 1, 100, 2**23, 2**24, 2**24+1, 2**25, 1e9, 1e15, 1e20, 1e30, 3e38}` × sign × pad by `io/tests/test_ordering_properties.py::test_chunk_bounds_contain_the_footprint_at_every_magnitude`.
