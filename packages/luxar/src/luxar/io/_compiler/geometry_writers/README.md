...
3. **Spatial ordering**:
   - `build_points_ordering(positions, n_points, n_dims, radii_for_ordering, ctx.ordering_ctx, ctx.store)` is called unconditionally; it returns `ordering_data` when spatial indexing is enabled and `None` otherwise
