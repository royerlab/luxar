...
appears: `write_colors` and `write_scalars` (colormap scalars) are shared by all
four geometry types — Points, Lines, GSplats, and Mesh — while `write_positions`
and `write_positive_scalar` serve the three per-element types. Mesh encodes its
vertices, faces and normals through a direct `ArrayEncoder` call and has no
POSITIVE_SCALAR attribute.
