...
# Curve/grid/compound-sort primitives live in _ordering/; re-exported here so
# the historical public paths (luxar.io.ordering.<name>) keep resolving. Some
# are used only by external callers, not by the remaining module body.
...
# Public API of this module — defined here plus the primitives re-exported from
# _ordering/ above. Listed explicitly so the re-exports satisfy mypy's
# no-implicit-reexport and the facade contract is unambiguous.
__all__ = [
    "morton_encode_nd",
    "morton_encode_128bit",
    "hilbert_encode_nd",
