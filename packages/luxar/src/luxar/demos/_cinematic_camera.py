...
distortion, which is two different lenses in one image.

So the demos pin neither, and compose their poses for 63° instead. That is free
for a scene the viewer auto-frames, because the bounding-sphere fit divides by
``tan(fov / 2)`` and simply moves the camera in. It is NOT free for a demo that
states its own distance — a multiple of the data's extent, a fitted radius —
because such a pose specifies a DISTANCE, not a framing: at a fixed distance,
47° → 63° scales the subject to 0.71x linear, which is HALF its screen area.
