    ...
    with asection("Writing Luxar PPI flow scene"):
        ...
        with LuxarZarrCompiler(output_path) as compiler:
            ...
            scene.add_points(
                ...
                colors=node_colors,
                radii=node_radii,
                sharpness=np.full(len(node_positions), 3.8, dtype=np.float32),
                opacity=0.94,
            ...
            scene.add_points(
                ...
                colors=hub_colors,
                radii=hub_radii,
                sharpness=np.full(len(hub_positions), 2.2, dtype=np.float32),
                opacity=0.82,
            ...
            if len(edge_vertices) > 0:
                scene.add_lines(
                    ...
                    widths=edge_widths,
                    colors=edge_colors,
                    sharpness=np.full(len(edge_vertices), 1.3, dtype=np.float32),
                    line_type="segments",
                    blending_mode="additive",
                    opacity=0.18,
