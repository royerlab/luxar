...
JUSTIFIED: dict[str, str] = {
    "demo_4d_fractals.py": (
        ...
        "1,000,000, jointly unsatisfiable. The 297 partition children are cut by "
        "ARRAY INDEX at the authored 150,000-element cap, not per plane — measured, "
        "221 of them span more than one (fractal, w) coordinate and they run "
        "69,233-150,000 points — so each falls under the gate's 200,000 threshold "
        "outright, and 37x below the 5,591,040 Points cap. RECORDED TRADE, not a "
        "defect: the gate does not force this shape. Roughly 30 laddered parts of "
        "~1,482,308 clear every arm too (rung 0 at the 12.5% share = 185,289, "
        "largest increment 741,152 under the 1,000,000 cap, 3.8x under the Points "
        "cap), while 297 parts cost about one request per part to bootstrap — the "
        "cost `_lod_policy`'s own module docstring warns about. 150,000 was kept "
        "because it needs no per-part ladder at all."
    ),
