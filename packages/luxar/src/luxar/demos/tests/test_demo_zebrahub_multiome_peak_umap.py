"""Regression tests for the Zebrahub demo's Points-LOD dependency gate.
...
core dependency — so a warm-cache, dependency-free run died with a
``ModuleNotFoundError`` instead of producing a viewable scene. The fix gates LOD
on both and falls back to flat Points, so the demo runs with either one blocked.

``create_zebrahub_scene`` takes ``(output_path, coordinates, attributes,
category_maps)`` directly, so these tests fabricate in-memory inputs and need no
cache, network, or zarr store.
"""
...
def test_scene_builds_flat_without_lod_dep(
    ...
    out = tmp_path / "zebrahub.luxar.zarr"
    got = create_zebrahub_scene(out, coordinates, attributes, category_maps)
