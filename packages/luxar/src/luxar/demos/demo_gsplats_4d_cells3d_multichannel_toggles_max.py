#!/usr/bin/env python3
"""GSplats Demo: 5D Multi-Channel Cells with Boolean Toggles — MAX BLENDING

Variant of the multichannel boolean-toggle demo that uses **max blending**
instead of additive blending. In max blending the brightest Gaussian value
along each ray wins, producing a maximum-intensity-projection (MIP) style
rendering rather than the summed volumetric look of the additive variant.

================================================================================
MAX BLENDING — BRIGHTEST WINS
================================================================================
...
=====================
  ...
  - **Additive** (original variant): Intensities sum along the ray, producing
    a bright volumetric glow where many Gaussians overlap.
  - **Max** (this variant): Only the peak Gaussian value along the ray is kept.
    This gives sharper, more defined structures — similar to a maximum intensity
    projection (MIP) in microscopy.
...
5. **Visualize** with max blending — Toggle each channel independently

USAGE:
======
    python demo_gsplats_4d_cells3d_multichannel_toggles_max.py [--no-cache] [--no-serve] [--serve-only]
...
Output:
    - Scene saved to: demos/gsplats_5d_cells3d_multichannel_toggles_max.zarr
...
# =============================================================================
# Scene Creation — Boolean toggle dimensions with MAX blending
# =============================================================================
...
def create_luxar_scene(gsplats_list, output_path=None):
    ...
    Uses **max blending** — only the peak Gaussian value along each ray is
    kept, producing a maximum-intensity-projection (MIP) style rendering.
    ...
    if output_path is None:
        output_path = (
            get_demos_output_dir() / "gsplats_5d_cells3d_multichannel_toggles_max.zarr"
        )

    # Map each channel to its own dimension name and the other channel's dim
    CHANNEL_DIM_NAMES = [ch["name"].lower() for ch in CHANNELS]

    with asection("Creating 5D Luxar Scene (boolean toggles, max blending)"):
        ...
        with LuxarZarrCompiler(
            ...
            scene.attrs["title"] = "GSplats: 5D Cells (max blending, boolean toggles)"
            scene.attrs["description"] = """
5D Multi-Channel Gaussian Splatting — Max Blending + Boolean Toggles
=====================================================================

Maximum-intensity-projection (MIP) variant of the multichannel demo.
Uses max blending: only the brightest Gaussian value along each ray is
kept, producing sharper, more defined structures compared to additive
blending.
...
Blending:
  - Max blending — brightest Gaussian wins (MIP-like)
  - Compare with additive variant for volumetric glow
  ...
  - Mouse drag to rotate, scroll to zoom, right-click drag to pan
            ...
            for i, (gsplats, ch_config) in enumerate(zip(gsplats_list, CHANNELS)):
                ...
                with asection(f"Adding {ch_name} (toggle: {own_dim})"):
                    ...
                    # Uses MAX blending for MIP-style rendering.
                    scene.add_gsplats(
...
def main():
    """Main demo execution."""
    aprint("=" * 70)
    aprint("GSplats Demo: 5D Multi-Channel Cells (max blending, boolean toggles)")
    aprint("=" * 70)
    aprint("3D per-channel fitting + independent boolean toggles + max blending")
    aprint("")
    ...
    # Create 5D scene with boolean toggles and max blending
