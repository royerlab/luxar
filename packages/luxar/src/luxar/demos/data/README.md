...
license, files, sizes, and checksums. There are two checksum contracts per file:
`sha256` describes the copy this repository ships, and the optional
`hosted_sha256` (with `hosted_bytes`) describes what the Zenodo record serves
where a refit made the two differ. It is generated by
