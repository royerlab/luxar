...
a refit made the two differ. Nothing under `zenodo` is shipped here any more, so
those entries carry only `sha256` and it *is* the hosted digest — the download
leg verifies against it. It is generated by
...
need renaming on the record. `zebrafish_4d.gsplats.zarr.zip` is on the
