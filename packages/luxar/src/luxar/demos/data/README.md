...
> generation is present. Before re-pinning a pair, run the slow
> `test_positional_pair_alignment.py` check from the staging checkout with the
> newly generated bytes under `delme/`. The manifest is generated by
