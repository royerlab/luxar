...
> `superseded_sha256` lists earlier pinned digests, newest last. It is generated by
> `scripts/gen_data_manifest.py` (`make gen-data-manifest`, gated in CI by
> `hatch run check-data-manifest`) and read at
> runtime by `luxar.demos._support.datasets.data_fetch.ensure_dataset`. The tables below are
...
`luxar.demos._support.datasets.data_fetch.local_fit_path`, which the fetch never inspects. Under
