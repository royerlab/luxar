...
| `gsplats_visible_human_head/` | 3D Visible Human head, true-color cryosections (NLM) | 1 `.gsplats.zarr.zip` + `vh_head_colors.npz` (per-splat RGB, indexed positionally against the fit; the pair is verified on load and agrees at 1.0 — it was resampled against the shipped store to fix #1670) |
