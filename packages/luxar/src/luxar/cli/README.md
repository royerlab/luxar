...
An integer `--seeds K` is a **whole-volume** budget (what a default `gsplat cal` reports as K\*): a tiled fit — `--tiling uniform`, a large `--tiling auto` volume, a `--tile k/M` worker — divides it across the tiles that survive floor subtraction and Hann windowing rather than giving each tile the full count. A uniform `batch-fit` plan measures foreground at one shared 10%-of-plan-range level and weights each tile as `Hann voxels × foreground_fraction^saturation_exponent`, then divides K proportionally with deterministic rounding. This keeps splat density independent of tile size for homogeneous content while retaining the calibrated `K ~ features^alpha` ratio between equal-sized tiles. An alpha fitted by `cal --fit-exponent` is a region-scale law measured on absolute feature counts; uniform weighting deliberately reuses it only as a density exponent, with tile size remaining linear at fixed density. The weighting preserves K whenever K is at least the non-empty tile count; a smaller K gives one seed per non-empty tile. Occupancy weighting and overlap-dominated sliver folding are currently `batch-fit` only; standalone `fit --tiling uniform` and `fit --tile k/M` retain the historical grid and equal-share fallback until #2838. A float ratio in `(0, 1]` is scale-free and applied per tile unchanged, so it also scales a narrow edge tile by its voxel count; `--tiling content` ignores `--seeds` entirely (per-box budgets come from the density plan).
