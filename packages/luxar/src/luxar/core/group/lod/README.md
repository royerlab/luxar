...
`resolve_lod_ladder` is the sole implementation; the four adders delegate to it
rather than restating it, and `core/tests/group/lod/test_lod_selector_contract.py`
pins both halves plus the consistency invariant (a stamped selector must agree
with its own stamped thresholds — mislabelling raises nothing, it just puts the
viewer's metric on the wrong scale).
