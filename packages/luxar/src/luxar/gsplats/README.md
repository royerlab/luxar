...
- `fold_tile_slivers`: Fold an overlap-dominated trailing tile into its predecessor. Defaults to `False` for library compatibility; the CLI passes `True` so direct and batch uniform grids agree.
- `tile_seed_counts`: Optional exact non-negative integer count per resolved tile. This overrides `seeds` tile by tile and is how the CLI carries occupancy-weighted whole-volume budgets into the sequential loop.
