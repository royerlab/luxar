...
- `grid_bsp_tree(specs, *, scale=None)` -- Serialized `bsp_tree` over a uniform tile grid, so the viewer paints tile-parts back-to-front instead of by centroid (#1555). `scale` is a per-axis, strictly positive factor mapping the specs' voxel frame onto the frame the splats actually live in (see below).
- `resolve_grid_scale(ndim, *, downscale_factors=None, voxel_size=None, output_space="real")` -- Builds that `scale`. The two terms COMPOSE: a `--downscale` grid is computed on the decimated shape while the workers rescale their splats back to full resolution, and a `voxel_size` fit with `output_space="real"` emits physical centers. Returns `None` when both are no-ops (#1587). Raises on an `output_space` outside `("real", "voxel")` -- silently reading a typo as "voxel" would drop the spacing term, which is the very mismatch this closes -- and on a `voxel_size` that is neither a scalar nor length-`ndim`. Every producer of a uniform tile grid's `bsp_tree` goes through it: `fit_tiled`, `dispatch_parallel_tiled`, and the `batch-fit` uniform merge (which re-resolves the workers' recorded `--preset`/`--config`).
