...
def preprocess_data(config: FitConfig) -> PreprocessedData:
    ...
    Performs normalization, candidate generation, and gradient dilution compensation.
    ...
    centers_overcomplete = config.centers_overcomplete

    # Auto-candidate generation if not provided
    if centers_overcomplete is None:
        centers_overcomplete = _generate_candidates(V, config.verbose)
    ...
    max_abs_error = _set_convergence_threshold(
        config.max_abs_error, config.verbose
    )
...
def _generate_candidates(V: np.ndarray, verbose: bool) -> np.ndarray:
    """Generate candidate centers using multiscale approach."""
    ...
    if verbose:
        aprint(
            f"Auto-generating candidates: {peaks_per_scale} peaks/scale for {V.size:,} pixels"
        )
...
def _set_convergence_threshold(
    max_abs_error: float | None, verbose: bool
) -> float:
...
def _calculate_gradient_dilution_compensation(
    ...
    return effective_lr, gradient_dilution_factor, dimensional_complexity, parameter_complexity
