...
1. **Background floor suppression** (`config.floor`, default `"auto"`): subtracts a
   ...
   recorded on `PreprocessedData.floor`; `specimen` also records its selected or
   fallback branch on `PreprocessedData.floor_strategy`. The floor is NOT added
   back (output amplitudes are background-relative).
