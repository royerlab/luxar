...
1. **Background floor suppression** (`config.floor`, default `"auto"`): subtracts a
   constant background pedestal / DC offset before normalization by pinning the
   effective `image_min` to the resolved floor (`_resolve_floor` → `estimate_floor`). `auto` = capped
