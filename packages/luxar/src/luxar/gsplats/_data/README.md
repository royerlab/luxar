...
- **`io_adapter.py`** — `IOAdapterMixin`: `GSplatData.save(path, ordering="hilbert", ...)` and `GSplatData.load(path)` — .gsplats.zarr I/O adapter. `save()` delegates to `write_gsplats_tree` in `luxar.gsplats.io.save_gsplats`; `load()` delegates to `luxar.gsplats.io.load_gsplats`. Sentinel `_USE_DEFAULT_COMPRESSOR` distinguishes "not specified" (→ Blosc) from explicit `compressor=None` (→ uncompressed zarr).
...
`GSplatData` directly composes four domain mixins (`RenderMixin`, `IOAdapterMixin`, `FilteringMixin`, `CullingMixin`) in `luxar.gsplats.gsplat_data`. `_GSplatDataOps` and `_SplatArrayMixin` are inherited transitively through those bases, so the mixins' cross-domain `self.` calls resolve through `_GSplatDataOps`'s stubs.
