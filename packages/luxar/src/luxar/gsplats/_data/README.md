...
- **`test_filter_per_level.py`**: Per-level rebuild for both `FilteringMixin.filter` and `CullingMixin.cull` — that reducing a laddered dataset keeps every substitutive level present and consistent.
