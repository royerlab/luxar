...
- **`intensity.py`** — `IntensityMixin`: Amplitude / color edits, ladder-preserving except where noted: `affine_intensity`, `normalize_intensity`, `clamp_intensity`, `scale_intensity`, `reweight_amplitude(multiplier)` (per-splat), `soft_scale_filter(highpass=..., lowpass=..., width=...)` (smooth log2-scale band reweighting), `with_colors(colors)`, the categorical-channel pair `with_label_ids(label_ids, label_vocabulary)` / `without_label_ids()` (attach and strip the exact per-splat class channel — the latter is what the refusal messages on decimation, substitutive LOD and INRIA export point callers at; `with_label_ids` refuses a multi-substitutive pyramid, since coarse splats merge fine ids) and the shared `_with_new_amplitudes(...)`.
