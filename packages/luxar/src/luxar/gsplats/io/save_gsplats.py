...
def save_gsplats(
    ...
    if colors is not None:
        ...
        # Validate color_mode for float colors
    ...
    # Write centers (COORDINATE type - never broadcast)
    encoder.encode(
        data=centers,
    ...
    # Write amplitudes (POSITIVE_SCALAR type)
    encoder.encode(
        data=amplitudes,
        zarr_group=splats_group,
        name="amplitudes",
    ...
    # Write cholesky_factors (CHOLESKY type)
    encoder.encode(
    ...
    # Write colors (COLOR type) - optional
    if colors is not None:
