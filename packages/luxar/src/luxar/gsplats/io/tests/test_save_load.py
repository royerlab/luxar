...
class TestSaveGsplats:
    ...
    def test_stamped_content_hash_preserves_float_array_attrs(self) -> None:
        from luxar._zarr_compat import create_array, memory_group
        from luxar.gsplats.io.save_gsplats import _stamp_content_hash
        ...
        assert lower[1]["min"] == np.nextafter(value, -np.inf)
        assert upper[1]["min"] == np.nextafter(value, np.inf)

    def test_stamped_content_hash_canonicalizes_float_group_attrs(self) -> None:
        ...
        def stamped(direction: float) -> tuple[str, dict[str, object]]:
            ...
            root.attrs.update(
                {
                    "type": "gsplats",
                    "fitting": {"final_loss": np.nextafter(value, direction)},
                }
            )
            content_hash = _stamp_content_hash(root)
            return content_hash, dict(root.attrs)["fitting"]

        lower = stamped(-np.inf)
        upper = stamped(np.inf)

        assert lower == upper
        assert lower[1]["final_loss"] == float(f"{value:.12g}")
