...
def write_supersplat_ply(path: Path, gt: GroundTruth) -> None:
    """Write a SuperSplat compressed .ply (single chunk, 18 chunk properties)."""
    n = gt.positions.shape[0]
    if n > 256:
    ...
    tc = norm(gt.colors.astype(np.float64), cmin, cmax)
    packed_color = (
    ...
    chunk = np.concatenate([pmin, pmax, smin, smax, cmin, cmax]).astype("<f4")
    vertex = np.stack(
