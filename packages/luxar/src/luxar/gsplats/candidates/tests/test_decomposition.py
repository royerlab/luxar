...
class TestInputValidation:
    ...
    def test_find_candidates_input_validation(self) -> None:
        """Test input validation in find_candidates_multiscale_gaussian."""
        V = np.random.randn(10, 10)

        # Test empty input
        with pytest.raises(ValueError, match="cannot be empty"):
            find_candidates_multiscale_gaussian(np.array([]))

        # Test scalar input
        with pytest.raises(ValueError, match="at least 1 dimension"):
            find_candidates_multiscale_gaussian(5.0)

        # Test invalid scales
        with pytest.raises(ValueError, match="non-empty sequence"):
            find_candidates_multiscale_gaussian(V, scales=[])

        with pytest.raises(ValueError, match="positive"):
            find_candidates_multiscale_gaussian(V, scales=[0.5, -1.0, 2.0])

        # Test invalid parameters
        with pytest.raises(ValueError, match="peaks_per_scale must be positive"):
            find_candidates_multiscale_gaussian(V, peaks_per_scale=0)

        with pytest.raises(ValueError, match="between 0 and 100"):
            find_candidates_multiscale_gaussian(V, percentile_thresh=150.0)

        with pytest.raises(ValueError, match="min_distance must be positive"):
            find_candidates_multiscale_gaussian(V, min_distance=-1.0)
...
class TestDecompositionCandidates:
    """Test find_candidates_from_decomposition function."""

    def test_basic_functionality_2d(self) -> None:
        """Test basic candidate generation on 2D synthetic image."""
        ...
        assert np.all(candidates[:, 0] <= img.shape[0])
        assert np.all(candidates[:, 1] >= 0)
        assert np.all(candidates[:, 1] <= img.shape[1])
    ...
    def test_ignore_finest_k_parameter(self) -> None:
        ...
        candidates_k0 = find_candidates_from_decomposition(
            img,
            scales=[1, 2, 4],
            ignore_finest_k=0,
        ...
        candidates_k1 = find_candidates_from_decomposition(
            img,
            scales=[1, 2, 4],
            ignore_finest_k=1,
            decompose_kwargs={"n_iters": 50, "verbose": False},
        ...
        # With k=0, may find more candidates (including noise from finest scale)
        # With k=1, should find fewer candidates (ignoring finest scale)
        # Note: This is a stochastic test, so we just check shapes are correct
        assert candidates_k0.shape[1] == 2
        assert candidates_k1.shape[1] == 2

    def test_min_distance_deduplication(self) -> None:
        """Test that min_distance properly deduplicates candidates."""
        ...
        candidates_small_dist = find_candidates_from_decomposition(
            img,
            scales=[1, 2],
            ignore_finest_k=0,
            min_distance=1.0,
        ...
        candidates_large_dist = find_candidates_from_decomposition(
            img,
            scales=[1, 2],
            ignore_finest_k=0,
            min_distance=10.0,
        ...
        # Larger min_distance should result in fewer or equal candidates
        assert len(candidates_large_dist) <= len(candidates_small_dist)
    ...
    def test_threshold_rel_filtering(self) -> None:
        ...
        candidates_high_thresh = find_candidates_from_decomposition(
            img,
            scales=[1, 2],
            ignore_finest_k=0,
            threshold_rel=0.5,
        ...
        # Low threshold should find more candidates
        candidates_low_thresh = find_candidates_from_decomposition(
            img,
            scales=[1, 2],
            ignore_finest_k=0,
            threshold_rel=0.05,
        ...
        assert len(candidates_low_thresh) >= len(candidates_high_thresh)

    def test_peaks_per_scale_limiting(self) -> None:
        """Test that peaks_per_scale limits candidates per scale."""
        # Create busy image with many features
        np.random.seed(42)
        img = np.random.rand(64, 64)
        img = img + 0.5  # Shift up to create many potential peaks

        candidates_unlimited = find_candidates_from_decomposition(
            img,
            scales=[1, 2, 4],
            ignore_finest_k=1,
        ...
        candidates_limited = find_candidates_from_decomposition(
            img,
            scales=[1, 2, 4],
            ignore_finest_k=1,
        ...
        # Limited should find fewer or equal candidates
        assert len(candidates_limited) <= len(candidates_unlimited)

    def test_1d_image(self) -> None:
        """Test candidate generation on 1D signal."""
        ...
        assert np.all(candidates[:, 0] >= 0)
        assert np.all(candidates[:, 0] <= len(signal))

    def test_3d_volume(self) -> None:
        """Test candidate generation on 3D volume."""
    ...
    def test_empty_result_handling(self) -> None:
        ...
        # May have 0 or very few candidates

    def test_input_validation(self) -> None:
        """Test input validation for find_candidates_from_decomposition."""
        valid_img = np.random.rand(32, 32)

        # Test empty input
        with pytest.raises(ValueError, match="cannot be empty"):
            find_candidates_from_decomposition(np.array([]))

        # Test scalar input
        with pytest.raises(ValueError, match="at least 1 dimension"):
            find_candidates_from_decomposition(5.0)

        # Test invalid scales
        with pytest.raises(ValueError, match="non-empty list"):
            find_candidates_from_decomposition(valid_img, scales=[])

        with pytest.raises(ValueError, match="positive"):
            find_candidates_from_decomposition(valid_img, scales=[1, -2, 4])

        # Test invalid ignore_finest_k
        with pytest.raises(ValueError, match="non-negative"):
            find_candidates_from_decomposition(valid_img, ignore_finest_k=-1)
        ...
        with pytest.raises(ValueError, match="positive"):
            find_candidates_from_decomposition(valid_img, min_distance=-1.0)

        # Test invalid threshold_rel
        with pytest.raises(ValueError, match="between 0 and 1"):
            find_candidates_from_decomposition(valid_img, threshold_rel=1.5)

        with pytest.raises(ValueError, match="between 0 and 1"):
            find_candidates_from_decomposition(valid_img, threshold_rel=-0.1)
