...
// ============================================================================
// Raw-parameter splat-centric path for unconstrained 3D Metal training
// ============================================================================

kernel void rasterize_forward_raw_splat_centric_3d(
    device const float* raw_mu [[buffer(0)]],
    device const float* raw_L_diag [[buffer(1)]],
    device const float* L_off [[buffer(2)]],
    device const float* raw_a [[buffer(3)]],
    device const float* sigma_min_diag [[buffer(4)]],
    device atomic_float* output [[buffer(5)]],
    constant uint3& shape_dhw [[buffer(6)]],
    ...
    if (tid == 0) {
        ...
        float l11 = sigma_min_diag[1] + stable_softplus(raw_L_diag[base3 + 1]);
        float l20 = L_off[base3 + 1];
        float l21 = L_off[base3 + 2];
        float l22 = sigma_min_diag[2] + stable_softplus(raw_L_diag[base3 + 2]);
        ...
        int cache_base = int(splat_id) * 24;
        cache[cache_base + 0] = s_center[0];
        cache[cache_base + 1] = s_center[1];
        cache[cache_base + 2] = s_center[2];
        cache[cache_base + 3] = l00;
        cache[cache_base + 4] = l10;
        cache[cache_base + 5] = l11;
        cache[cache_base + 6] = l20;
        cache[cache_base + 7] = l21;
        cache[cache_base + 8] = l22;
        cache[cache_base + 9] = s_conic[0];
        cache[cache_base + 10] = s_conic[1];
        cache[cache_base + 11] = s_conic[2];
        cache[cache_base + 12] = s_conic[3];
        cache[cache_base + 13] = s_conic[4];
        cache[cache_base + 14] = s_conic[5];
        cache[cache_base + 15] = s_amp;
        cache[cache_base + 16] = s_truncate_sq;
        cache[cache_base + 17] = float(s_lo[0]);
        cache[cache_base + 18] = float(s_lo[1]);
        cache[cache_base + 19] = float(s_lo[2]);
        cache[cache_base + 20] = float(s_extent[0]);
        cache[cache_base + 21] = float(s_extent[1]);
        cache[cache_base + 22] = float(s_extent[2]);
        cache[cache_base + 23] = float(s_total_voxels);
    }
...
kernel void rasterize_backward_raw_splat_centric_3d(
    device const float* grad_output [[buffer(0)]],
    device const float* raw_mu [[buffer(1)]],
    device const float* raw_L_diag [[buffer(2)]],
    device const float* L_off [[buffer(3)]],
    device const float* raw_a [[buffer(4)]],
    device const float* sigma_min_diag [[buffer(5)]],
    device float* d_raw_mu [[buffer(6)]],
    device float* d_raw_L_diag [[buffer(7)]],
    device float* d_L_off [[buffer(8)]],
    device float* d_raw_a [[buffer(9)]],
    constant uint3& shape_dhw [[buffer(10)]],
    constant uint& n_splats [[buffer(11)]],
    constant float& truncate [[buffer(12)]],
    constant float& intensity_floor [[buffer(13)]],
    ...
    if (tid == 0) {
        ...
        d_raw_L_diag[base3 + 2] = dl22 * stable_sigmoid(raw_L_diag[base3 + 2]);
        d_L_off[base3 + 0] = dl10;
        d_L_off[base3 + 1] = dl20;
        d_L_off[base3 + 2] = dl21;
        d_raw_a[splat_id] = tg_amp[0] * stable_sigmoid(raw_a[splat_id]);
    }
}


kernel void rasterize_backward_raw_cached_splat_centric_3d(
    device const float* grad_output [[buffer(0)]],
    device const float* raw_mu [[buffer(1)]],
    device const float* raw_L_diag [[buffer(2)]],
    device const float* L_off [[buffer(3)]],
    device const float* raw_a [[buffer(4)]],
    device const float* sigma_min_diag [[buffer(5)]],
    device const float* cache [[buffer(6)]],
    device float* d_raw_mu [[buffer(7)]],
    device float* d_raw_L_diag [[buffer(8)]],
    device float* d_L_off [[buffer(9)]],
    device float* d_raw_a [[buffer(10)]],
    constant uint3& shape_dhw [[buffer(11)]],
    constant uint& n_splats [[buffer(12)]],
    constant float& truncate [[buffer(13)]],
    constant float& intensity_floor [[buffer(14)]],
    constant float& shift_C [[buffer(15)]],
    constant float& inv_one_minus_C [[buffer(16)]],
    constant uint& grad_output_is_scalar [[buffer(17)]],
    uint3 tg_pos [[threadgroup_position_in_grid]],
    uint tid [[thread_index_in_threadgroup]]
) {
    uint splat_id = tg_pos.x;
    ...
    if (tid == 0) {
        int cache_base = int(splat_id) * 24;
        s_center[0] = cache[cache_base + 0];
        s_center[1] = cache[cache_base + 1];
        s_center[2] = cache[cache_base + 2];

        s_L[0] = cache[cache_base + 3];
        s_L[1] = cache[cache_base + 4];
        s_L[2] = cache[cache_base + 5];
        s_L[3] = cache[cache_base + 6];
        s_L[4] = cache[cache_base + 7];
        s_L[5] = cache[cache_base + 8];

        s_conic[0] = cache[cache_base + 9];
        s_conic[1] = cache[cache_base + 10];
        s_conic[2] = cache[cache_base + 11];
        s_conic[3] = cache[cache_base + 12];
        s_conic[4] = cache[cache_base + 13];
        s_conic[5] = cache[cache_base + 14];
        s_amp = cache[cache_base + 15];
        s_shift_C = shift_C;
        s_inv_one_minus_C = inv_one_minus_C;
        s_truncate_sq = cache[cache_base + 16];
        s_lo[0] = int(cache[cache_base + 17]);
        s_lo[1] = int(cache[cache_base + 18]);
        s_lo[2] = int(cache[cache_base + 19]);
        s_extent[0] = int(cache[cache_base + 20]);
        s_extent[1] = int(cache[cache_base + 21]);
        s_extent[2] = int(cache[cache_base + 22]);
        s_total_voxels = int(cache[cache_base + 23]);
    }
