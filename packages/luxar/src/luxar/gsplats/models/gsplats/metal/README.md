...
| | status |
|---|---|
| **Compiles** | Partially gated. `make check-native` / `hatch run check-native` checks every translation unit when its toolchain is present, using the standards shipped by the build. CI's Linux `python-tests` requires only the host-C++ arm. The Objective-C++ binding and Metal shader are checked manually on Apple silicon before each release with `hatch run check-native --require cxx --require metal`; hosted macOS CI and a dedicated Mac runner are intentionally not used. No GPU is required. |
| **Numeric parity vs the torch reference** | Tests EXIST and are substantial — 59 test functions here, 138 on the CUDA side, including `test_metal_numerical.py` and `test_metal_conic.py`. They are skip-guarded on Metal availability, so **they never run in CI**, which has no GPU. The release checklist requires the Metal suite on an Apple-silicon box; confirm it passes without MPS-unavailable skips. |
