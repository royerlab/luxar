#!/usr/bin/env python3
"""
Coins image Gaussian splatting demo with interactive compression analysis.

This demo applies Gaussian splatting to the classic scikit-image coins dataset,
...
on metallic textures with circular objects and varying illumination gradients.
"""
...
with asection("Coins Gaussian Splatting Demo"):
    aprint("🪙 Interactive compression analysis on classic coins image")
    aprint("💰 Metallic textures, circular objects, and illumination gradients")

    with asection("Loading and preprocessing data"):
        # Load coins image (already grayscale)
        img = data.coins()  # Grayscale (303, 384)
        aprint(f"Original coins image: {img.shape}, range=[{img.min()}, {img.max()}]")

        # Convert to float32 and normalize to good working range
        V = img_as_float32(img) * 100.0  # Scale to [0, 100] (same as mitosis/astronaut)

        # No cropping - use the full image to show all coins
        # The image is already a good size (303x384) for demo purposes

        aprint(f"Preprocessed coins image: {V.shape}")
        aprint(f"Data range: [{V.min():.4f}, {V.max():.4f}]")
        aprint("🎯 Full image with multiple coins and metallic surface textures")

    # Configure dynamic operations (use defaults like mitosis/astronaut)
    ...
    with asection(f"Fitting Gaussian splats ({N_ITERS} iterations)"):
        ...
        aprint(f"🎉 Fitted {len(result.amplitudes)} splats to reconstruct coins image")
...
if not NO_NAPARI:
    ...
    viewer = napari.Viewer(title="Coins Gaussian Splatting Demo")
    ...
    # Add residual stack with copper colormap for metallic theme
    viewer.add_image(
        np.abs(stack_resid),
    ...
    # Shapes & points that update with slider - use gold colors for coin theme
    ...
    def _set_overlay_text(t_index: int) -> None:
        ...
        viewer.text_overlay.text = (
            f"🪙 Coins Demo | Kept splats: {K}/{N}  |  Model bits: {bits_model:,}  "
