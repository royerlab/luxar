...
- **`generate_fit_sbatch()`** — Generate the main sbatch array job script for fitting, with support for sequential or parallel task packing, preemptible requeue, on-the-fly denoising, and preprocessed denoised input. Parallel packing scales the fit allocation from the per-task CPU/RAM flags and pins each worker's GPU, thread count, and quality-memory share. Both packing modes retain per-task failures and make the array element exit non-zero after all assigned tasks have been joined or attempted. Output paths and preset values are quoted as literal arguments; percent signs in the output directory are doubled in the log directives so Slurm's filename-pattern expansion leaves them literal; output directories containing line terminators or NUL are rejected because Slurm directives are line-oriented.
...
| `merge_orchestrator.py` | Post-batch merge: default streaming `kind=partition` (one part per tile); `--flat` for the legacy single-leaf fan-in (tiles -> per-(T,C) -> per-C -> final). Uniform partition split planes are rebuilt from the manifest's voxel-space tile grid; content partitions reuse the shared plan's box tree. A `voxel_size` in the run's `--config`, or the explicit `--physical` mode using discovered NGFF spacing, moves worker output into a different frame, so the planner records `grid_scale` and the merge scales either representation accordingly (#1587, #2764). A config `downscale:` does not change that output frame: every task rescales back to the full-resolution frame the planner tiled, and a decimating value that makes a uniform worker grid differ from the planned one is refused at plan time (#1624). An absent `grid_scale` means the frames agree. A recorded physical frame also refuses `--refine volume` because each part's crop is taken in source voxels. |
