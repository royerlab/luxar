...
def merge_batch_results(
    ...
    return _merge_partition(manifest, output_dir, channel_colors, force, verbose)
...
def _merge_partition(
    ...
    # Single tile (K=1) → emit a bare leaf, NOT a 1-part partition.
    if n_k == 1:
    ...
    def _parts() -> "Iterator[GSplatNode]":
        ...
        for k in range(n_k):
            ...
            # or a lod group of leaves). Hand the tree node straight to the
            # streaming writer; it writes part_<i>/ then releases the splats.
            node = part.tree
    ...
    with asection(f"Streaming spatial partition: {n_k} tiles -> parts"):
        # NOTE: this path deliberately NEVER calls GSplatData.concatenate across
        # all tiles. Each part is built + written + released in turn, so peak
        # memory is one tile-region (the OOM fix that motivates tiling).
        ...
        if verbose:
            aprint(f"  Wrote kind=partition with {n_written} parts")
