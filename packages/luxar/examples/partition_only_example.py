...
2. ``manual_partition`` — an explicit ``scene.add_partition_group(...)`` with a
...
def main() -> None:
    ...
    with LuxarZarrCompiler(
        output_path, auto_partition_max_elements=8_000
    ) as compiler:
        ...
        # 2. Manual partition: hand-built add_partition_group, then a single
