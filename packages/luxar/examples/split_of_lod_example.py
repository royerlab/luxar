#!/usr/bin/env python3
"""Split-of-LOD Example — kind=split wrapping kind=lod, with UX polish.

This example demonstrates the four UX-polish items landed in PR #319:

1. **Auto-split heuristic** — ``LuxarZarrCompiler(auto_split_max_elements=N)``
   automatically wraps oversize ``add_points`` / ``add_gsplats`` inputs
   in a ``kind=split`` Group. User-explicit ``split=`` always wins.
2. **Combined badge** — the layers panel shows ``N parts × M LODs`` on
   the wrapper header when a Split layer contains nested LOD groups.
3. **Broadcast LOD dropdown** — the "Active level" dropdown on the
   Split wrapper updates every nested ``lod_group`` in lock-step. Ragged
   ladders clamp per-group automatically.
4. **Split-aware picking** — hovering an inner ``part_<i>`` reports the
   outermost ``kind=split`` wrapper's path (matches the layers-panel
   "outermost-as-layer" convention).

Layout: one wrapper layer ``ribbon`` containing a strip of points the
auto-split splits into several spatial parts; under each part the
...
def make_strip(n: int = 50_000, seed: int = 0):
    """A long horizontal strip — guarantees the BSP splits along X."""
...
def main() -> None:
    output_path = get_examples_output_dir() / "split_of_lod_example.zarr"

    # Auto-split kicks in when an add_points call exceeds the threshold.
    # 50k points / 12k cap → ~4 spatial parts.
    with LuxarZarrCompiler(
        str(output_path), auto_split_max_elements=12_000
    ) as compiler:
        scene = compiler.create_scene(dimensions=Dimensions.default_3d())

        # === 1. Auto-split Points wrapper ===
        # The user calls add_points as normal — no split= argument. The
        # compiler-level threshold kicks in, BSP partitions into N
        # spatial parts, and the result is a kind=split Group named
        ...
        # === 2. A hand-built Split-of-LOD layer ===
        ...
        # the broadcast dropdown renders only for a selected ``kind=split``
        # layer that wraps nested ``kind=lod`` groups. So this split group
        # itself carries ``layer=True`` — no plain-group wrapper needed.
        #
        # auto-split's children are bare leaves, so the ``ribbon`` layer
        # above can't surface the broadcast feature; this hand-built tree
        # mixes both: a kind=split wrapping two kind=lod ladders.
        outer = scene.add_split_group(
            "ribbon_lod",
            display_type="points",
            max_elements=12_000,
            layer=True,
        ...
        for i, (lo, hi) in enumerate([(-20.0, 0.0), (0.0, 20.0)]):
            ...
            #
            # ``split=False`` opts each LOD level out of the compiler's
            # auto-split heuristic. Without it the ~24k-point ``lod_fine``
            # leaf exceeds ``auto_split_max_elements`` and gets wrapped in
            # a kind=split group — which *drops* the ``min_pixel_size``
        ...
        aprint(
            "Created two layers:\n"
            "  - ribbon: 50k points auto-split into N spatial parts "
            "(combined badge: 'N parts').\n"
            "  - ribbon_lod: hand-built kind=split wrapping 2 nested "
            "kind=lod groups\n"
