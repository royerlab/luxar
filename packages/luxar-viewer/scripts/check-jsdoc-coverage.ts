...
class JSDocChecker {
  ...
  printResults(summary: Summary): boolean {
    ...
    if (failedFiles.length > 0) {
      ...
      failedFiles.forEach(result => {
        ...
        console.log(`     Coverage: ${result.coverage.toFixed(0)}% (${result.documented}/${result.exports})`);
        console.log(`     Need ${Math.ceil((this.threshold / 100) * result.exports) - result.documented} more JSDoc comments\n`);
      });
    }
    ...
    Object.entries(byPackage)
      ...
      .forEach(([pkg, stats]) => {
        ...
        console.log(`  ${icon} ${pkg}: ${stats.coverage.toFixed(0)}% (${stats.documented}/${stats.exports})`);
      });
   ...
   */
  private groupByPackage(results: CoverageResult[]): Record<string, { exports: number; documented: number; coverage: number }> {
    ...
    const packagesWithCoverage: Record<string, { exports: number; documented: number; coverage: number }> = {};
