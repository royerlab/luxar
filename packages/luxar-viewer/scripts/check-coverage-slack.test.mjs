...
describe('evaluate', () => {
  ...
  it('reports a floor the measurement has fallen below', () => {
    const { failures } = evaluate(summary, { lines: 95 }, { lines: 90 }, ROOT, 3, 1);
  ...
  it('warns when measurement erodes beyond the recorded budget', () => {
    const { failures, warnings } = evaluate(summary, { lines: 88 }, { lines: 91.01 }, ROOT, 3, 1);
    expect(failures).toEqual([]);
  ...
  it('does not warn at exactly the erosion budget', () => {
    const { warnings } = evaluate(summary, { lines: 88 }, { lines: 91 }, ROOT, 3, 1);
    expect(warnings).toEqual([]);
  });
});
