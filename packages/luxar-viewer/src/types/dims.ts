 ...
 *
 * @internal — no call site left in the viewer; it was only ever reachable
 * through the (now removed) `types` barrel.
 */
 ...
 *
 * @internal — no call site left in the viewer; it was only ever reachable
 * through the (now removed) `types` barrel. Note `SceneDimsManager` has a
 * same-named *method* that reads cached ranges off the manager; the
 * `sceneDimsManager.getDimensionRanges()` calls in `ui/`, `core/` and `input/`
 * are that method, not this function.
 */
