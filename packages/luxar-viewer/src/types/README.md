...
`line-primitive.ts` owns the line rendering primitive selection (issue #1352): `'capsule'` (the DEFAULT since the flip) shades a gaussian-like profile of the 2D point-to-segment distance — stable end-on discs, seamless bisector-partitioned joints, quad-class cost; `'screen-space'` is the flat quad, the leaner path for very large line scenes (the third primitive, `volumetric`, was deleted after the flip — `rendering/materials/line/README.md` has the history). Unlike `line-join.ts` there is no authored node attribute: the primitive is a renderer implementation choice — `?linePrimitive=` override, then the `Advanced → Line primitive` policy setting, whose `auto` mode folds the scene graph into its aggregate concurrent effective load before material construction and resolves each node against that shared value via `resolveLinePrimitiveForNode` (plain `resolveLinePrimitive(explicit?)` serves harnesses that never run bootstrap). Same layering rationale as `line-join.ts` below.
...
A `Group` whose `kind === 'lod'` selects **one** of N alternative children at runtime by comparing the group's on-screen size against each child's `coverage_fraction` threshold, in the units the group's `selector` attr names. Under `'screen-area'` — what every DERIVED ladder stamps — a threshold is a literal screen-area fraction: the group's projected bbox rect area over the viewport area, so the ladder is derived by occupancy halving (`[0, …, 1/8, 1/4, 1/2]`, full detail while the node occupies at least half the screen, one level coarser per halving of occupied area), and a partition-bound ladder anchors its finest one step higher, at `1.0` (the tile alone fills the screen).
...
// attrs.selector is 'screen-area' (derived ladders) or 'coverage' (legacy);
// attrs.display_type is the user-facing label;
// attrs.default_level seeds the manual-override widget (0-based, coarsest-first).
```

- **`LODGroupMetadata`** -- `{ type: 'group', kind: 'lod', selector: 'screen-area' | 'coverage', display_type?, default_level?, ... }`. Each child carries a `coverage_fraction` threshold (a `number`; 0.0 coarsest, then under `'screen-area'` 0.5 for a whole-object ladder's finest and 1.0 for a partition-bound one's, while under legacy `'coverage'` the finest is 1.0 whole-object and up to 4.0 == `SCREEN_FILL_DIAGONAL_RATIO / FILL_FACTOR` partition-bound — a range the format enforces, not the viewer), strictly monotonic increasing coarsest→finest; the selector picks the finest child whose threshold is satisfied.
...
- **`ZarrSceneAttrs`** -- Root scene group attributes: the 0.2 header (`format_version`, `format_type`, `luxar_software_version`; the 0.1 legacy `luxar_version` is read as a fallback), `content_hash`, scene dimensions, units, position bounds, and an optional `viewer_config`. Checked against the contract's `attr_keys` at the type level (`ROOT_ATTR_KEYS_TYPED`); the dimension entry keys are locked to `dimension_attr_keys` (`DIMENSION_ATTR_KEYS_MATCH_CONTRACT`) and the affine `nd_transform` keys to `nd_transform_affine_keys`.
...
- **`LoaderType`** -- `'point-spatial-index' | 'lines-spatial-index' | 'gsplats-spatial-index' | 'mesh-whole-node'`. Mesh loads whole (no spatial index), hence the distinct member.
...
- **`GlobalStats`** -- aggregate dashboard numbers across all loaders: a `dataset*`/`visible*` pair per geometry type (points, segments, splats, and `datasetTriangles`/`visibleTriangles` for mesh), `droppedElements` (elements omitted by the renderer's element-texture capacity clamp — points/lines/gsplats only, since mesh is not element-texture backed), queries-per-second, and `recommendations`; per-tier cache hit rates live in `CacheMetrics`, not here.
...
- **`GeometryCounters`** -- `Record<GeometryTypeName, number>`, the monitor's per-geometry-type aggregation shape. The element _nouns_ (points / segments / splats / triangles) deliberately stay on `GlobalStats`' named fields: each is rendered with its own label and DOM id, so they are presentation, not aggregation.
