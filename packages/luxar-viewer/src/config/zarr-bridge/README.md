...
- `viewer-config-utils.ts` — Direction-conversion utilities and key mappings.
  - `RENDERING_SETTINGS_MAP` — snake_case zarr key → camelCase `RenderingSettings` key, covering tone mapping, EOG (exposure / global offset / global gamma), bloom, navigation, cinematic mode, vignette, detector noise, anti-aliasing (FXAA/MSAA/SSAA), chromatic lens distortion, fly controls, dynamic clipping, adaptive DPR, and the high-DPR ceiling.
  - `REVERSE_SETTINGS_MAP` — built at module load by inverting `RENDERING_SETTINGS_MAP`.
  - `extractRenderingOverrides(zarrConfig)` — returns a `Partial<RenderingSettings>` containing only fields set in zarr. Also pulls `camera.fov`, `camera.fov_preset`, `camera.near`, and `camera.far` because those properties live on `RenderingSettings` even though zarr stores them under `camera`. A recognized `camera.fov_preset` resolves to its configured numeric FOV when `camera.fov` is absent; `Custom` and unrecognized names do not. A numeric `camera.fov` that disagrees with a recognized preset by at least 0.5° wins and logs once per `preset:fov` pair via `_warnedFovPresetConflicts`. Finally, when `cinematic_mode === true` (strictly — `false`/`null`/absent expand nothing), the cinematic preset from [`../cinematic-preset.ts`](../cinematic-preset.ts) is expanded into every preset key the scene did **not** set explicitly, so an authored flag actually renders the film look. Author-set keys always win (a key counts as author-set exactly when it is already in the overrides object), and expanding here means both consumers — `applyZarrDefaults` and `buildResetDefaults` — get it for free. `fov` and `fovPreset` are the one exception to the per-key rule: they are a coupled framing pair, so setting either one in zarr suppresses both. The warning set is exported so tests can reset it.
  - `renderingSettingsToZarr(settings)` — reverse mapping; only a partial inverse of `extractRenderingOverrides`, since the cinematic expansion adds keys the author never wrote and makes the pair asymmetric. Camera-related fields (`fov`, `fovPreset`, `near`, `far`) are intentionally not emitted here — `captureViewerState` writes them under `camera.*`. Iterates the input keys (not `REVERSE_SETTINGS_MAP`) so a `RenderingSettings` field missing from the bridge map is still dropped (round-trip safety with older zarr files) but logs a single warning per unknown key via the module-local `_warnedUnknownRenderingKeys` set, which is exported so tests can reset it.
...
- `_warnedUnknownRenderingKeys`, `_warnedFovPresetConflicts` — module-local warning sets, exported for tests.
