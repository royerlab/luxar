...
export interface DataLoadingPerformanceConfig {
   ...
   *
   * - `null` (default): **auto-size** from `navigator.deviceMemory`, measured
   *   JS heaps up to 1 GiB, and an explicit cache-pool signal, with a 2 GB
   *   ceiling and a 512 MB fallback only when no signal exists. An
