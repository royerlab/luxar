...
- `data.ts` — `dataLoadingPerformanceConfig: DataLoadingPerformanceConfig`. Defines the four optimization pipelines:
  ...
  - **GPU Buffer Pool**: `useGPUBufferPool` (true), `gpuPoolMaxSize` (20), `gpuPoolEvictionFrames` (300), `gpuPoolEvictBatchSize` (5), `gpuPoolMaxBytes` (512 MB byte-budget; `0` disables).
