...
- `data.ts` — `dataLoadingPerformanceConfig: DataLoadingPerformanceConfig`. Defines the four optimization pipelines:
  ...
  - **GPU Buffer Pool**: `useGPUBufferPool` (true), `gpuPoolMaxSize` (20), `gpuPoolEvictionFrames` (300), `gpuPoolEvictBatchSize` (5), `gpuPoolMaxBytes` (single GPU-geometry byte budget shared by the pool + LOD retention; `null` = the lower of 25% of `navigator.deviceMemory` and the heap model's full non-cache remainder, with a 2 GB ceiling and a 512 MB fallback only when neither signal exists; explicit `?cacheBudgetMB=` replaces the heap-derived remainder in either direction, while mobile keeps an independent 128 MiB peer minimum; `0` = disable, positive = pin; overridable at startup via `?gpuBudgetMB=` or the embedded app/layer `gpuPoolMaxBytes` options).
