...
- `data.ts` — `dataLoadingPerformanceConfig: DataLoadingPerformanceConfig`. Defines the four optimization pipelines:
  ...
  - **GPU Buffer Pool**: `useGPUBufferPool` (true), `gpuPoolMaxSize` (20), `gpuPoolEvictionFrames` (300), `gpuPoolEvictBatchSize` (5), `gpuPoolMaxBytes` (single GPU-geometry byte budget shared by the pool + LOD retention; `null` = the lower of 25% of `navigator.deviceMemory`, the working-set share of a measured JS heap up to 1 GiB, and one third of explicit `?cacheBudgetMB=`, with a 2 GB ceiling and a 512 MB fallback only when no signal exists; without `deviceMemory`, the cache-derived value replaces that fallback in either direction; `0` = disable, positive = pin; overridable via `?gpuBudgetMB=`).
