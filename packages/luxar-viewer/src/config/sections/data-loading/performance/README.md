...
- `data.ts` — `dataLoadingPerformanceConfig: DataLoadingPerformanceConfig`. Defines the four optimization pipelines:
  ...
  - **GPU Buffer Pool**: `useGPUBufferPool` (true), `gpuPoolMaxSize` (20), `gpuPoolEvictionFrames` (300), `gpuPoolEvictBatchSize` (5), `gpuPoolMaxBytes` (single GPU-geometry byte budget shared by the pool + LOD retention; `null` = the lowest estimate from 25% of `navigator.deviceMemory`, the measured JS heap, the mobile device class, and one third of explicit `?cacheBudgetMB=`, with a 2 GB ceiling and a 512 MB fallback only when no signal exists; `0` = disable, positive = pin; overridable at startup via `?gpuBudgetMB=` or the embedded app/layer `gpuPoolMaxBytes` options).
