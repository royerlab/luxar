...
- `src/cache/multi-level-caching-store/fetch-retry.ts` — `fetchWithRetry` reads `retryAttempts` (becomes `maxAttempts = retryAttempts + 1`) and `timeoutMs` (split into `ceil(timeoutMs / maxAttempts)` per-attempt header and no-progress windows). Each attempt may spend one window reaching headers and then up to the longer of eight windows or `Content-Length / 16 KiB/s` reading the body, plus retry backoff. With the defaults, a body up to 960 KiB can therefore keep one key active for about 270 seconds across four attempts, not 30 seconds.
