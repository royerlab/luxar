...
UI configuration slice. Owns z-index layering for overlays, timing constants for transient UI (error auto-dismiss, help-close debounce), loading-spinner geometry, the debug-console panel/interceptor/style settings, the scale-bar overlay, and per-component border-radius/padding tokens for the dataset browser, debug console, rendering-controls panel, and data monitor.
