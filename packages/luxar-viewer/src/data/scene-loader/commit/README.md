...
| `commit-mesh-geometry.ts`     | Synchronous Mesh commit. Much shorter than the other three by design, not because it is unfinished: a mesh has **no GPU buffer pool** (its vertex buffers are uploaded once per `displayDims` epoch and never resized) and **no capacity clamp** (its element ordinal is `gl_VertexID`, bounded at the loader's preflight). It finds the placeholder by name, updates the geometry in place via `updateMeshGeometry`, applies the epoch's material `side` AND shading variant (`applyMeshSide` / `applyMeshShading` — both follow `displayDims`, not just the authored attrs), and still invalidates the render object, driven by `updateMeshGeometry`'s rebind flag rather than by a pool. Depth-sort registration IS shared with the other three (face centroids as the "center", `setCommittedData` as the resolve-path gate) — only the APPLY differs, permuting `geometry.index` instead of `aSortedIndex`; see `rendering/depth-sort-coordinator/triangle-ordering.ts`.                                                      |
