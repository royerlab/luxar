...
| `commit-mesh-geometry.ts`     | Synchronous Mesh commit. No GPU buffer pool and no capacity clamp — a mesh's vertex buffers are uploaded once per `displayDims` epoch instead of being resized per slice, so `updateMeshGeometry` rewrites the whole node in place. Also applies the epoch's `side` and shading variant, and updates `visibleTriangleCount` / `visibleVertexCount`. Calls `invalidateRenderObjectFor` only when that rewrite reports an attribute rebind, so a pure slice move skips it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **Synchronous only.** No `await`, no microtask hops. Commit runs
  ...
  `process/data-processor-{lines,gsplats,mesh}.ts` modules.
- **Four-geometry symmetry.** Each commit module exports a single
  `commit{Points|Lines|GSplats|Mesh}Geometry` function with the same
  ...
  matches the single export. Mesh keeps every step but the pool
  branch — it has no pooled buffers, so it invalidates off the
  whole-node update's own attribute-rebind flag instead.
