...
| `commit-mesh-geometry.ts`     | Synchronous Mesh commit. Much shorter than the other three by design, not because it is unfinished: a mesh has **no GPU buffer pool** (its vertex buffers are uploaded once per `displayDims` epoch — plus once per LEVEL for a reveal ladder, which writes its growing prefix into the same capacity-sized buffers rather than resizing them, #1521/#1522) and **no capacity clamp** (its element ordinal is `gl_VertexID`, bounded at the loader's preflight). It finds the placeholder by name, updates the geometry in place via `updateMeshGeometry`, applies the epoch's material `side` AND shading variant (`applyMeshSide` / `applyMeshShading` — both follow `displayDims`, not just the authored attrs), and still invalidates the render object, driven by `updateMeshGeometry`'s rebind flag rather than by a pool. Stamps `userData.committedVertexCount` from the data actually received, because `position.count` is capacity-sized for a ladder and would otherwise report the lifetime total from level 0 on — `core/app/debug/debug-state.ts` reads the stamp instead. Depth-sort registration IS shared with the other three (face centroids as the "center", `setCommittedData` as the resolve-path gate) — only the APPLY differs, permuting `geometry.index` instead of `aSortedIndex`; see `rendering/depth-sort-coordinator/triangle-ordering.ts`. |
...
- **Per-geometry symmetry.** Each commit module exports a single
  ...
  `displayDims` epoch — plus once per LEVEL for a reveal ladder, which
  writes its growing prefix into the same capacity-sized buffers rather
  than resizing them, #1521/#1522), so it updates the geometry in
  ...
  (`visibleTriangleCount` / `visibleVertexCount` / `committedVertexCount`).
