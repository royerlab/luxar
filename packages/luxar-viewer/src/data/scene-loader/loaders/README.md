...
| `loader-registry.ts`    | `LoaderRegistry` class — holds the three per-geometry loader maps (`loaders`, `linesLoaders`, `gsplatLoaders`), the `failedLoaders` error-tracking map, plus registration (incl. the defensive `unregisterXLoader` no-ops — lazy substitutive LOD levels never join the sweep, so the release-path unregister only guards hypothetical future registering paths) / lookup / `recordFailure` / `clearFailure` / `disposeAll` helpers.                                                                                                                                                                                                                                              |
