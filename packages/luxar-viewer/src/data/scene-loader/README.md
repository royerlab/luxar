...
| progressive (`refinement.ts`)          | The generic progressive-LOD refinement loop (`runProgressiveRefinement`) shared by all four geometry types via their `data/{points,lines,gsplats,mesh}/lod-refinement.ts` wrappers: per-frame rAF yield, view-state-queue cancellation handoff, per-loader processing, and lock release when no LODs remain or a successful pass makes no progress. No README.                                                                                                                                                                                                                                                                                                                                                      |
| progressive (`refinement-wrapper.ts`)  | Shared wrapper plumbing for all four geometry refinement loops: candidate admission, abort/failure handling, settled-residency recording, and progress callbacks. Keeps the admission gate's decline reasons coupled to the callback exclusion set so declined loaders cannot keep the loop alive. No README.                                                                                                                                                                                                                                                                                                                                                                                                       |
| progressive (`residency-budget.ts`)    | Settled-residency ceiling shared by the four refinement sweeps. It refuses unaffordable rungs, gives admitted loaders a per-pass share of remaining headroom, and reconciles that reservation with measured growth. Tracks sweep-registered progressive leaves across geometry types; lazy `lod_group` levels are outside this accounting. Beyond the gate, `RefinementResidencyReporter` also keeps a durable `RefinementResidencyStop` record of every byte-ceiling refusal — loader-scoped and cumulative within that life, contract documented on the type — exported through the debug snapshot (`getState().refinementResidency`) so a capture tool can refuse a scene that stopped short (#2508). No README. |
| progressive (`density-gate.ts`)        | Projected-density rung gate consulted after byte admission: defers an additive rung while the node exceeds its drawing-buffer-pixel cap (`config.densityGuard`; tighter for max/normal/opaque). Shares the budget's `declined` set for the run, re-evaluates at run start, and re-kicks from the frame walk when screen coverage grows. Meshes bypass it so reveal ladders complete. The app pipeline injects the provider (`scene/projected-density.ts`). No README.                                                                                                                                                                                                                                               |
