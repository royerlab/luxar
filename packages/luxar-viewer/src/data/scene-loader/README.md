...
| progressive (`density-gate.ts`)        | Projected-density rung gate consulted by the residency budget for a rung the byte ceiling admits: defers the next additive rung while the node already projects more elements per drawing-buffer pixel than its cap (`config.densityGuard`; the tighter cap for max/normal/opaque). Shares the budget's `declined` set within a run; re-evaluated at every run start and re-kicked from the per-frame density walk once the node fills enough of the screen. Mesh nodes bypass the gate so reveal ladders always complete. Density comes from a provider the app pipeline injects (`scene/projected-density.ts`). No README. |
