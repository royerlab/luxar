...
| progressive (`residency-budget.ts`)    | Settled-residency ceiling shared by the four refinement sweeps. It refuses unaffordable rungs, gives admitted loaders a per-pass share of remaining headroom, and reconciles that reservation with measured growth. Tracks sweep-registered progressive leaves across geometry types; lazy `lod_group` levels are outside this accounting. Beyond the gate, `RefinementResidencyReporter` also keeps a durable `RefinementResidencyStop` record of every byte-ceiling refusal — cumulative across refinement runs for the life of the loader, never reset within it — exported through the debug snapshot (`getState().refinementResidency`) so a capture tool can refuse a scene that stopped short (#2508). No README. |
