 ...
 *
 * Callsites consume the registry by discriminating on `handler.kind`,
 * which keeps each branch correctly typed without a top-level generic.
 *
 * The third-party type slots (`zarr.Location`, `LoaderFactoryDeps`,
 * `ProcessCtx`, `CommitCtx`) intentionally stay `unknown` at this
 * layer to avoid pulling zarrita + loader-factory imports into the
 * shared types module; concrete handlers re-narrow them.
 */
export interface GeometryTypeHandler<
  TLoader = unknown,
  TLoaded = unknown,
  TStaged = unknown,
  TViewState = unknown,
> {
  ...
  /**
   * Build the per-node loader (typically a spatial-index loader) for
   * the given scene-graph node, paired with its zarr location handle
   * and a handler-specific dependency bundle (caches, monitor, etc.).
   */
  createLoader(node: SceneNode, loc: unknown, deps: unknown): TLoader;

  /**
   * Derive the per-type view-state slice from the loader-wide base
   * view-state. Each handler narrows `base: ViewState` to its
   * type-specific view-state shape (e.g. attaches dimension metadata,
   * applies extend_to_all tolerance overrides).
   */
  deriveViewState(node: SceneNode, base: ViewState): TViewState;

  /**
   * Transform freshly-loaded data into staged commit data (used by
   * Lines + GSplats which need an off-main-thread projection pass).
   * For Points this is a no-op and returns `void`.
   */
  process(loaded: TLoaded, node: SceneNode, ctx: unknown): TStaged | void;

  /**
   * Apply the staged commit data to the GPU pool / scene graph. Runs on
   * the main thread after `process` completes; the optional session is
   * the active UpdateSession used for profiling.
   */
  commit(staged: TStaged, node: SceneNode, ctx: unknown, session?: UpdateSession): void;
}
