...
The `luxar-viewer.data` package provides the client-side data loading infrastructure for visualizing massive nD point cloud and line datasets stored in Zarr format. It implements efficient spatial index-based loading, intelligent caching, nD slicing algorithms, and hierarchical scene management.

**Core Responsibility**: Read and decode Zarr datasets generated by the Python `luxar.io` package, enabling real-time visualization of massive datasets (points and lines) through spatial indexing and progressive loading.
