...
type ExactUnsignedArray = Uint8Array | Uint16Array | Uint32Array | BigUint64Array;

function exactIdAt(values: ExactUnsignedArray, index: number): string {
  const value = values[index];
  return typeof value === 'bigint' ? value.toString() : String(value);
}

export function compactGSplatLabelIds(
  values: ExactUnsignedArray,
...
export function projectGSplatLabelIndices(
  ...
  for (let index = 0; index < visibleCount; index++) {
    projected[index] = indices[sourceIndices[index]] ?? 0;
  }
  return projected;
}
