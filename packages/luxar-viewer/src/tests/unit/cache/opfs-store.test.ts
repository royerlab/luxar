...
// [cache.md/G7][P5] `compactOrderCounter` fires when `orderCounter > 1e12`
// (source line 385 / 583). The check fires on a long session and silently
// renumbers entries 0..N-1; a regression that compacted in the WRONG order
// would break LRU eviction across sessions. Drive the path directly via
// the same-named private method through `as unknown as { ... }`.
describe('OPFSStore.compactOrderCounter [cache.md/G7][P5]', () => {
...
// Helper for the G7 test block above — re-create the mock directory handle
// shape used elsewhere in this file. Kept local so this test file remains
