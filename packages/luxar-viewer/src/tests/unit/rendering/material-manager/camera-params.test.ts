 ...
 * (resolution, isOrtho, nearCull, pixelRatio) and broadcasts to registered
 ...
 * resize / projection-kind event ("splats missing until the camera moves" on scenes
...
describe('MaterialManager camera params on newly created materials', () => {
  ...
  it('applies the stored nearCull to a line material created after updateCameraParams', () => {
    ...
    mm.updateCameraParams(new THREE.Vector2(800, 600), false, 0.42, 1);
  ...
  it('applies the stored nearCull to a mesh material created after updateCameraParams', () => {
    // Mesh joined the broadcast with the near fade (#1431). It reads only the
    // near-cull input of the contract (its ortho test reads the camera being
    // drawn with), and the same "created after camera setup" hazard
    ...
    const mm = new MaterialManager();
    mm.updateCameraParams(new THREE.Vector2(800, 600), true, 0.42, 1);
  ...
  it('applies the stored resolution/isOrtho to materials created after updateCameraParams', () => {
    const mm = new MaterialManager();
    mm.updateCameraParams(new THREE.Vector2(1234, 777), true, 0.5, 1);
    ...
    // GSplats read the projection kind in shader: no ortho uniform to carry.
    expect(gsplat.uniforms.uIsOrtho).toBeUndefined();
  });
