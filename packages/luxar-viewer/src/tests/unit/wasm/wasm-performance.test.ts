...
describe.skipIf(!wasmFilesExist)('WASM Performance Benchmarks', () => {
  ...
  describe('compute_nd_visibility_points', () => {
      ...
      benchmark(
        ...
        () => {
          ...
          for (let i = 0; i < numPoints; i++) {
            ...
            positions[i * ndim + 3] = Math.random() * 10; // Hidden dim
            radii[i] = 1.0 + Math.random();
          }
          ...
          return {
            wasm: wasmModule,
            ts: tsModule,
            args: { positions, radii, slicePos, tolerance, ndim, numPoints, output },
          };
        },
        (wasm, { positions, radii, slicePos, tolerance, ndim, numPoints, output }) => {
          wasm.compute_nd_visibility_points(
            positions,
            radii,
            slicePos,
            tolerance,
        ...
        (ts, { positions, radii, slicePos, tolerance, ndim, numPoints, output }) => {
          ts.compute_nd_visibility_points(
            positions,
            radii,
            slicePos,
            tolerance,
    ...
    it(`should benchmark with ${SMALL_SIZE.toLocaleString()} points`, () => {
      ...
      const result = results[results.length - 1];
      console.log(
        `  compute_nd_visibility_points (${SMALL_SIZE.toLocaleString()}): ` +
          `WASM=${result.wasmTimeMs.toFixed(3)}ms, ` +
          `TS=${result.tsTimeMs.toFixed(3)}ms, ` +
          `speedup=${result.speedup.toFixed(2)}x`
      );
    ...
    it(`should benchmark with ${MEDIUM_SIZE.toLocaleString()} points`, () => {
      ...
      const result = results[results.length - 1];
      console.log(
        `  compute_nd_visibility_points (${MEDIUM_SIZE.toLocaleString()}): ` +
          `WASM=${result.wasmTimeMs.toFixed(3)}ms, ` +
          `TS=${result.tsTimeMs.toFixed(3)}ms, ` +
          `speedup=${result.speedup.toFixed(2)}x`
      );
    ...
    it(`should benchmark with ${LARGE_SIZE.toLocaleString()} points`, () => {
      ...
      const result = results[results.length - 1];
      console.log(
        `  compute_nd_visibility_points (${LARGE_SIZE.toLocaleString()}): ` +
          `WASM=${result.wasmTimeMs.toFixed(3)}ms, ` +
          `TS=${result.tsTimeMs.toFixed(3)}ms, ` +
          `speedup=${result.speedup.toFixed(2)}x`
      );
