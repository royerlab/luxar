...
| **DEPTH SORT**         | `sort_splats_by_depth` — back-to-front ordering for correct alpha blending.                                                                                                                                                   | Budget: ≥ 50 M splats/s, report-only by default and enforced by `pnpm test:perf:strict` on a quiet host. Reports absolute throughput.                       |
...
The depth-sort benchmark has a **≥ 50 M splats/s** budget (via `perf-budget.test.ts`). That test is excluded from the default unit suite (see `vitest.config.ts`) and runs only under the opt-in perf suite. `pnpm test:perf` reports a breach without failing; use `pnpm test:perf:strict` (`LUXAR_PERF_QUIET_HOST=1`) to enforce absolute floors on a quiet host.
...
- `vitest.config.ts` — excludes `perf-budget.test.ts` from the default suite; `pnpm test:perf` reports the depth-sort ≥ 50 M splats/s floor, while `pnpm test:perf:strict` enforces it on a quiet host.
