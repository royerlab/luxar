...
├── global-setup.ts                # Vitest global setup (auto-generates fixtures; builds/rebuilds the WASM artifact)
└── global-setup-perf.ts           # Vitest global setup for the perf suite (reuses `ensureWasmBuilt`, no fixtures)
...
- `helpers/` — Small utilities shared across specs: array comparison (`array-compare.ts`), seeded randomness (`random.ts`), projection adapters, line-join metrics, `make-test-node-build-ctx.ts` — the shared `NodeBuildCtx` stub factory the scene-loader node specs build their context from, so a new required member of that interface is added in one place instead of five — `quiet-host-throughput.ts` — the opt-in policy for absolute performance floors — and `wasm-artifact.ts` — the single loader for the built `public/wasm/` artifact (see `src/wasm/README.md`). No README of its own.
- [`benchmarks/`](./benchmarks/README.md) — Micro-benchmarks for hot paths (partially tracked).
- `screenshots/` — Playwright specs that capture README/doc images and the gallery media (`generate-readme-images.spec.ts`, `generate-doc-images.spec.ts`, `generate-gallery.spec.ts`), plus pure decision modules the gallery harness splits out of its spec so they can be unit-tested without a browser: `exposure-policy.ts` (auto-exposure, see `gallery-exposure-policy.test.ts` under `unit/`), `crop-policy.ts` (the border-lit "is the subject cropped?" warning, see `gallery-crop-policy.test.ts`), `frame-similarity.ts` (the still-vs-orbit-frame correlation, see `gallery-frame-similarity.test.ts`), `orbit-axis.ts` (which signed world axis the rock revolves about, see `gallery-orbit-axis.test.ts`), `gallery-selection.ts` (`GALLERY_ONLY` ids plus the README-derived alias, see `gallery-selection.test.ts`) and `gallery-timelapse-settle.ts` (the progressive-refinement settle predicate and bounded wait, see `unit/screenshots/gallery-timelapse-settle.test.ts`). No README of its own.
...
- `setup.ts` — Vitest per-file setup: installs all mocks via `installAllMocks()`, patches `Reflect.construct` so Vitest can `new`-construct arrow-function mocks, and silences jsdom's "Not implemented: navigation" jsdomError so download-via-`<a>.click()` paths (EXR/video/screenshot export) don't pollute the console.
- `global-setup.ts` — Vitest **global** setup (one-shot per run). Parses the fixture list from `tests/fixtures/generate_test_data.py` (single source of truth) through `tools/fixture-manifest.ts`, the shared parser the Playwright preflight reads too, regenerates any missing zarr fixtures via `hatch run fixtures:python ... generate_test_data.py`, and regenerates `roundtrip_expectations.json` when stale. It also builds the compiled WASM artifact (`ensureWasmBuilt`) — missing, or present but no longer declaring every name in `src/wasm/required-exports.ts` — so a stale `public/wasm/` is normally rebuilt before any test loads it. Without a Rust/wasm-pack toolchain (or if the build fails) it warns and leaves the stale artifact in place; the loaders then abort by name rather than silently skipping.
- `global-setup-perf.ts` — Vitest **global** setup for the perf suite (`vitest.perf.config.ts`). Reuses the same `ensureWasmBuilt` and nothing else: the perf benches read no zarr fixtures, and a bench box often lacks the hatch/Python environment the generator needs.
