...
- `screenshots/` — Playwright specs that capture README/doc images and the gallery media (`generate-readme-images.spec.ts`, `generate-doc-images.spec.ts`, `generate-gallery.spec.ts`), plus `exposure-policy.ts` — the gallery harness's pure auto-exposure decision, split out of the spec so it can be unit-tested without a browser (see `gallery-exposure-policy.test.ts` under `unit/`). No README of its own.
