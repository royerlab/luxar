...
- `screenshots/` — Playwright specs that capture README/doc images and the gallery media (`generate-readme-images.spec.ts`, `generate-doc-images.spec.ts`, `generate-gallery.spec.ts`), plus two pure decision modules the gallery harness splits out of its spec so they can be unit-tested without a browser: `exposure-policy.ts` (auto-exposure, see `gallery-exposure-policy.test.ts` under `unit/`) and `crop-policy.ts` (the border-lit "is the subject cropped?" warning, see `gallery-crop-policy.test.ts`). No README of its own.
