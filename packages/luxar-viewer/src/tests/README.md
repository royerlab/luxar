...
- `helpers/` — Small utilities shared across specs: array comparison (`array-compare.ts`), seeded randomness (`random.ts`), projection adapters, line-join metrics, and `wasm-artifact.ts` — the single loader for the built `public/wasm/` artifact (see `src/wasm/README.md`). No README of its own.
