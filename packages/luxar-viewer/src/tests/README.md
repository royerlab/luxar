...
- [`mocks/`](./mocks/README.md) — Browser API, THREE.js, OPFS, and WebGL2 mocks installed globally by `setup.ts`.
- [`benchmarks/`](./benchmarks/README.md) — Micro-benchmarks for hot paths (partially tracked).
- `screenshots/` — Playwright specs that capture README/doc images and the gallery media (`generate-readme-images.spec.ts`, `generate-doc-images.spec.ts`, `generate-gallery.spec.ts`). No README of its own.
