...
| `evaluateWithDeadline(evaluation, timeout, onTimeout)`                 | Bound one already-started `page.evaluate` — it carries no timeout of its own, so otherwise only the whole test budget stops it (#1651).                                           |
