...
| `captureElementScreenshot(page, selector, timeout?)`       | Capture the viewer canvas from its post-processed framebuffer; capture embed/host elements as composited PNGs while hiding unrelated DOM.                                                                                                             |
| `samplePixelAt(page, selector, fx, fy)` / `samplePixelsAt` | Decode one framebuffer-or-element capture and read one or many pixels from the same frame.                                                                                                                                                            |
| `captureCanvasRGBA(page, selector?)`                       | Decode one framebuffer-or-element capture into a full-frame RGBA buffer for whole-image / multi-region analysis on a single identical frame.                                                                                                          |
| `getElementPixelStats(page, ...)`                          | Pixel-statistics rollup over one framebuffer-or-element capture.                                                                                                                                                                                      |
