...
| `waitForWebGLError(page, predicate)`                                   | Block until accumulated WebGL errors satisfy a predicate. Each read is bounded by the remaining budget, and none is dispatched into a remainder shorter than one poll interval (a discarded read has already drained the GL queue). It never throws (it returns data) but warns once on give-up, saying whether any read answered. No call sites today. |
