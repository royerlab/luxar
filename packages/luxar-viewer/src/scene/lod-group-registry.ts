 ...
 * ``FILL_FACTOR × fittedAxisPx`` pixels — i.e. HALF of the **fitted screen axis**
 * (``fittedAxisPx = min(viewport.width, viewport.height)``), which a normal
 * full-frame view already exceeds. Coarser children (smaller fractions) step in
 * as the object shrinks below that. Lowering the factor shows the finest level
 * sooner, raising it later.
 *
