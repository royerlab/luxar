...
| `clipping-policy.ts`    | Three policy helpers operating on a narrow `ClippingCtx` (`camera`, `controls`, `scene`, `boundsCache`, `getSceneBoundsFromMetadata`): `applyClippingPlanes` (validates `near < far`, warns on `far/near > 10000`, calls `updateProjectionMatrix`, and reports whether it applied), `autoAdjustFromBounds` (metadata first, `THREE.Box3.setFromObject` fallback, also pushes scene diagonal into `controls.setSceneScale`, and returns `{ near, far, applied }`), and `updateDynamicFromCache` (per-frame sphere-based near/far, with a 0.1% change threshold to avoid projection-matrix thrash on sub-pixel camera moves).                                                                                                                                                                                                                                                                                |
