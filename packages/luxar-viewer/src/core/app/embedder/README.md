...
| `playSound(name)` / `stopSound(name)`                      | Start or stop one sound node by name (the last path segment, or the full path) regardless of its slab audibility; `stopSound` uses the node's fade-out. `playSound` returns `false` when the node is unknown, undecoded, muted, or still behind the autoplay gate; `stopSound` returns `false` when nothing was stopped.                                                                                                                                                                                         |
