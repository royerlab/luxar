...
| `'selection'`          | `SelectionPayload \| null`      | Fires when the hover-pick changes (`nodeName` + `elementIndex` + `hitNodeName` under the cursor, or `null` when cleared). Subscribe **before** `init()` to provision picking for all datasets. What `elementIndex` counts is per-node — see `SelectionPayload`: a labelled node reports an on-disk index — flat, or, for **points**, an additive-LOD ladder, whose per-level maps are composed into the parent's union CSR space (#1439); a labelled **lines** node reports the picked segment's start-vertex row. An unlabelled node — or a labelled additive-LOD **lines** ladder, which publishes no map across its levels — reports the visible-buffer slot. |
