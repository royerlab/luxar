...
| Method                                  | Description                                                                                                                                                                                                                                                                                                                                              |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `flyTo(pose, { durationMs?, easing? })` | Smooth transition to a `CameraSnapshot` (the shape `getCameraPose()` returns), interpolated in the orbit parameterisation (target lerp, direction slerp, log distance, up slerp) so it arcs around the scene. Any canvas pointer/wheel/touch or keydown cancels it where it is; so does a newer `flyTo()` or a dataset switch. Resolves `{ completed }`. |
