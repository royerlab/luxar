...
| `'selection'`          | `SelectionPayload \| null`      | Fires when the hover-pick changes (`nodeName` + `elementIndex` + `hitNodeName` under the cursor, or `null` when cleared). Subscribe **before** `init()` to provision picking for all datasets. What `elementIndex` counts is per-node — see `SelectionPayload`: a labelled flat node reports an on-disk index (a labelled **lines** node reports the picked segment's start-vertex row), while an unlabelled node — or a labelled additive-LOD ladder, which publishes no map across its levels (#1439) — reports the visible-buffer slot. |
