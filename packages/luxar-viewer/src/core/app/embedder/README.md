...
| `'selection'`          | `SelectionPayload \| null`      | Fires when the hover-pick changes (`nodeName` + `elementIndex` under the cursor, or `null` when cleared). Subscribe **before** `init()` to provision picking for all datasets. What `elementIndex` counts is per-node — see `SelectionPayload`: a labelled node reports an on-disk index (a labelled **lines** node reports the picked segment's start-vertex row), an unlabelled one the visible-buffer slot. |
