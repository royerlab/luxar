...
| `'dataset-fault'`      | `DatasetFaultPayload`           | Fires at most once when the loaded dataset becomes unable to update, while preserving its last complete frame. Subscribe before `init()` / `switchDataset()`, or call `getDatasetFault()` to inspect a fault that occurred before subscription.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
