...
so the rail control and `O` key remain responsive during a slow first load.
Selections made in that window are refused with a retry message; they do not
change the configured source or overlap the initial load.
