...
| `show-browser.ts`        | Constructs a `DatasetBrowser` modal and routes its callbacks through a `ShowDatasetBrowserPorts` interface. Clears the error overlay on open, normalizes the chosen URL (strips trailing slashes), optionally calls `replaceBrowserDataSourceUrl` when `updateBrowserUrl` is opted in, propagates the new src back to the orchestrator via `onSrcChange`, then dispatches into `loadDataset`. Hands a close handle to `InputHandler` so the Escape key routes through the browser's own `close()` rather than yanking the DOM node. Caller must guard against double-open.                                                                                                         |
| `browser-shortcut.ts`    | Installs a window-level `open-dataset-browser` custom-event listener via the supplied `EventGroup` (so it tears down on dispose). Calls `showBrowser()` only when `hasOpenBrowser()` is false — prevents modal stacking when the event fires twice.                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `load-dataset.ts`        | The post-load UI init pipeline — the longest helper in the folder. Runs ten steps in a strict order observable by users and tests: clear dimension UI → dispose previous overlays → `setSceneId` (must precede `loadSceneData` so persisted settings reach materials at construction time) → `loadSceneData` → push `viewer_config` into `renderingControls` (apply zarr defaults only when no localStorage entry exists) → update fly-speed range from scene scale → init dimension sliders + scale bar → init layers panel + colormap legend → init overlays + GPU picking → apply `viewer_config` UI state → optionally open the cache-stats monitor → kick the animation loop. |
...
LuxarApp.init(src)
  ...
  │            │
  │            └─ onDatasetSelect(url) → loadDataset(url, ports)
  │
...
`loadDataset` is also called by `showBrowser`'s `onDatasetSelect`
callback — every path that ends in a loaded scene funnels through the
same helper, so the ordering invariants apply uniformly.
...
- **Re-entry-safe shortcut.** `installBrowserShortcut` short-circuits
  when a browser is already open, so a stray custom event can't stack
  modals.
