...
| `browser-shortcut.ts`    | Installs a window-level `open-dataset-browser` custom-event listener via the supplied `EventGroup` (so it tears down on dispose). Toggles the modal: calls `closeBrowser()` when `hasOpenBrowser()` is true and `showBrowser()` otherwise; the orchestrator additionally suppresses `showBrowser()` while a dataset switch is active.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `load-dataset.ts`        | The post-load UI init pipeline — the longest helper in the folder. Runs thirteen steps in a strict order observable by users and tests: clear dimension UI → dispose previous overlays → `setSceneId` (must precede `loadSceneData` so persisted settings reach materials at construction time) → decide whether localStorage or scene defaults win → `loadSceneData` (allow the resolved scene FOV to participate in auto-framing only for first visits) → push `viewer_config` into `renderingControls` (apply the remaining zarr defaults under the same precedence decision) → update fly-speed range from scene scale → init dimension sliders + scale bar → init layers panel + colormap legend → init overlays + GPU picking → apply `viewer_config` UI state → optionally open the cache-stats monitor → kick the animation loop.                   |
...
installBrowserShortcut({ events, hasOpenBrowser, showBrowser, closeBrowser })
  └─ window 'open-dataset-browser'  → open if closed / close if open (toggle)
