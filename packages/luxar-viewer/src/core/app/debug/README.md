...
| `fps`                                                                                          | `AdaptiveDPRManager.getCurrentFPS()`                                 | Presented-frame rate; `undefined` until at least two samples exist. `renderer.info.render.frame` is a render-pass counter, not an FPS reading.                                                                                                                                                        |
...
| `getLodLoadStats()` / `resetLodLoadStats()`                                                    | `data/scene-loader/lod-load-stats`                                   | Per-stage timing for lazy loads plus additive per-level loads, keyed by geometry, level, and residency                                                                                                                                                                                                |
