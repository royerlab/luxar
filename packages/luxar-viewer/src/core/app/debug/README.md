...
| `getLodLoadStats()` / `resetLodLoadStats()`                                                    | `data/scene-loader/lod-load-stats`                                   | Per-stage timing for lazy loads plus additive per-level loads, keyed by geometry, level, and residency. Additive nodes of the same geometry type share keys; node paths are deliberately excluded.                                                                                                    |
