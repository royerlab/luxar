...
| `sceneManager` (EventDispatcher)          | `projection-changed`        | `pickingSystem.markDirty()` — a FOV edit reprojects the SAME camera and produces no controls `change`, so the cached buffer would otherwise answer at the old projection (#1916) |
