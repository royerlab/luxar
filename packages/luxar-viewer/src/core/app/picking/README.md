...
- **Additive ladders.** A progressive (additive) node today writes its CSR per `additive_<i>` sub-group, which is not a scene-graph node, while `points-progressive-loader.ts` concatenates every loaded level into one committed buffer — so labels on a laddered node are unusable regardless of `lookupPath`. The reader half of the fix is in place for **Points**: once the producer writes ONE union CSR on the PARENT node, keyed by `additive_0 || additive_1 || …` (#1422, not on main yet), the parent path indexes the committed buffer correctly, because the loader composes the levels' slot → on-disk maps into that union space (#1439). Lines ladders still wait on #1424, and gsplat ladders carry no labels at all.
- **Element id vs on-disk index — now resolved where the node can resolve it.** `result.elementId` reaches the handler already translated into the on-disk element index the CSR is keyed by, wherever the node can resolve one: `resolveOnDiskElementId` (`rendering/picking/picking-system/element-id-map.ts`) applies the published slot → on-disk map at the single `PickResult` construction site, and where the identity already holds no map is published and the raw slot already _is_ the on-disk index. Points and GSplats both resolve, for a node declaring `has_labels` / `has_image_labels`; Mesh needs none, since its `gl_VertexID` already is the on-disk vertex ordinal. Otherwise the id stays a _visible-buffer storage slot_ and diverges from the on-disk index as soon as a hidden dimension culls chunks (the loaded ranges no longer start at 0) or triggers effective-radius compaction (`data/points/projection.ts`). That now leaves Lines (#1424) — a per-segment slot against a per-vertex CSR, wrong even unsliced and shifted further by those two — and label-less Points / GSplats nodes, which have no CSR to miss, but an embedder reading `SelectionPayload.elementIndex` on one is reading a slot.
