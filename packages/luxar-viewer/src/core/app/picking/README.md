...
- **Element id vs on-disk index.** `result.elementId` is a _visible-buffer storage slot_ (`aSortedIndex`), not the on-disk element index the CSR is keyed by. The two coincide only while no dimension is hidden: a hidden dimension can cull chunks (so the loaded ranges no longer start at 0) and triggers effective-radius compaction (`data/points/projection.ts`), either of which shifts the slot. Plumbing the real ids through loader → commit → texel is pre-existing work tracked separately.
