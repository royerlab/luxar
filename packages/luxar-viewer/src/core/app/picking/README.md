...
2. **Label detection** — walks the `LuxarScene` group looking for `userData.attrs.has_labels` / `has_keys` / `has_image_labels`. No string/image channels **and** no embedder `selection` listener (`hasSelectionConsumer()`) → return four `undefined`s, picking stays off for this session. A selection listener alone still provisions picking; the loaders below are built only for the flags that were actually found.
