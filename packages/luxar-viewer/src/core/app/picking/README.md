...
- **Additive ladders.** A progressive (additive) node today writes its CSR per `additive_<i>` sub-group, which is not a scene-graph node, while `points-progressive-loader.ts` concatenates every loaded level into one committed buffer — so labels on a laddered node are unusable regardless of `lookupPath`, a producer-side gap (#1422). The reader half is in place for **Points**: once the producer writes ONE union CSR on the PARENT node, keyed by `additive_0 || additive_1 || …` (#1422, not on main yet), the parent path indexes the committed buffer correctly, because the loader composes the levels' slot → on-disk maps into that union space (#1439). A Lines ladder is still out — #1424 resolved the per-node segment→vertex chain, but nothing composes the LEVELS of a ladder — and gsplat ladders carry no labels at all.
