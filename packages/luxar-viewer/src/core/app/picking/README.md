...
- **Additive ladders.** A progressive (additive) node carries ONE union CSR on its parent node (#1422), spanning the levels in `additive_<i>` order — which is the same node `lookupPath` names, and the same space `points-progressive-loader.ts` produces when it concatenates the committed levels. So `lookupPath` indexes it correctly on a fully-loaded, unsliced layer. What is still missing is the slot → on-disk map ACROSS the ladder (each level's map is in that level's own space, so the concat drops it), so under chunk culling or effective-radius compaction the raw slot shifts and the tooltip names the wrong element (#1439).
