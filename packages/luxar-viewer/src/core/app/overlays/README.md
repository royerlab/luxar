...
4. `initOverlays({ disposePrevious, sceneManager, inputHandler, recordingPanel })` — looks up the `LuxarScene` group in `sceneManager.scene.children`, pulls `overlayConfigs`, `zarrBaseUrl`, and the narrow `readOverlayFile` store port from its `userData`, and asynchronously calls `manager.loadOverlays(...)` when the configs and URL are present.
