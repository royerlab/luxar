 ...
 *     who chose e.g. `pan-y` deliberately keeps their choice.
 ...
 *
 * All listeners register through the caller's {@link EventGroup}, so a single
 * `dispose()` removes them with everything else the app owns.
 *
...
export function installCanvasGestureOwnership(canvas: HTMLCanvasElement, events: EventGroup): void {
  if (touchActionIsDefault(canvas)) {
    ...
    // iOS long-press image/link callout and text selection over the canvas.
