...
| `probe-controller.ts` | U-shape probe lifecycle: arm on scale-down, settle after the probe window (accept / reject vs. the pre-change baseline), void on pause or display change |
| `bounds-ledger.ts` | Learned operating bounds: the rejected-probe floor and its TTL decay (evidence-based ceiling and rejection backoff join with the control-loop follow-ups) |
