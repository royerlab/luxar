...
| File              | Role                                                                                                                       |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `material.ts`     | GLSL3 `THREE.ShaderMaterial` wrapper (`PointPickingMaterial`). Camera-aware; tighter 50%-radius truncation than the visual material |
...
| `shaders.ts`      | GLSL3 vertex/fragment source strings + `POINT_PICK_SOURCE: ShaderSource`. Source-of-truth for the GLSL path; references `pick.tsl.ts` for the WebGPU branch |
