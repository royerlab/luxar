...
| `element-id-map.ts`   | `resolveOnDiskElementId(mainNode, slot)` — translates the visible-buffer storage slot the vote reports into the on-disk element index the label CSR is keyed by, using the `elementIdMap` stamp the commit pipeline wrote alongside `committedData`. Identity when no map applies (see the parent README). For a lines node the resolved value is the picked segment's START vertex row, since line labels are per-vertex |
