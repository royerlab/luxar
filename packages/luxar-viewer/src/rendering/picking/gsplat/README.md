...
| `material-tsl.ts` | WebGPU `NodeMaterial` counterpart (`GSplatPickingTSLMaterial`). Same `CameraAwareMaterial` contract                                                              |
...
| `shaders.ts`      | GLSL3 vertex/fragment source strings + `GSPLAT_PICK_SOURCE: ShaderSource`. Reuses `GLSL_SANITIZE_FUNCTIONS` + `invalidCov2D` parity guards from the visual side  |
