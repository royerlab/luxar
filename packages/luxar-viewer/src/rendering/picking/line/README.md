...
| File              | Role                                                                                                                                                                  |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `material.ts`     | GLSL3 `THREE.ShaderMaterial` wrapper (`LinePickingMaterial`). Honours `LUXAR_SHARPNESS_TWO` define for the sharpness=2 fast path; recompiles on `needsUpdate`          |
| `material-tsl.ts` | WebGPU `NodeMaterial` counterpart (`LinePickingTSLMaterial`). Same `CameraAwareMaterial` contract; rebuilds the TSL node graph on ortho flip or sharpness toggle      |
