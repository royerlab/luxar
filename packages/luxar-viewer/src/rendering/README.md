...
- A probe whose measurement window spans a content change is CONFOUNDED — its baseline was measured on the old content — so it may only conclude "stop here for now": the reduction is kept, the DPR is never reverted upward, the rejection-backoff streak is untouched, and the walk is held by a provisional floor carrying the short `contentChangeRecheckMs` TTL until content settles. Per-frame LOD churn on a slow loop therefore stops one step in instead of walking to `minDPR`, and learns nothing from a comparison that cannot mean anything
