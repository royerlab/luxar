...
**Cross-mesh (inter-part) ordering:** depth sorting orders splats WITHIN a mesh, but a `kind=partition` gsplat is multiple sibling meshes, and THREE orders transparent _objects_ by their `matrixWorld` origin — which is the shared world origin for every part (splat centers are baked into the geometry), so THREE's per-object key is identical and the parts would draw in fixed creation order, not back-to-front. So `evaluateDepthSortPerFrame` ALSO sets each visible normal-mode part's `renderOrder` every frame — THREE sorts transparent objects by `renderOrder` before `z`, ascending, so the lowest (farthest) draws first. Cleared to 0 when a mesh leaves `normal` mode.

The order comes from one of two paths:
...
Both run independent of the within-mesh re-sort hysteresis (cheap). Rank values order the parts among _themselves_; cross-_layer_ depth interleaving with independent transparent geometry is out of scope (the classical-import scenes this targets are single-layer). Even with a correct part order, splats whose footprints spill across a tile boundary can still interleave — only per-splat / single-mesh sorting is exact there. Additive/luminous/max are commutative so their order is irrelevant; normal-mode Points/Lines remain a non-goal.
