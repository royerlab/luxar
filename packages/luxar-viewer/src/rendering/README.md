...
**Cross-mesh (inter-part) ordering:** depth sorting orders splats WITHIN a mesh, but a `kind=partition` gsplat is multiple sibling meshes, and THREE orders transparent _objects_ by their `matrixWorld` origin — which is the shared world origin for every part (splat centers are baked into the geometry), so THREE's per-object key is identical and the parts would draw in fixed creation order, not back-to-front. So `evaluateDepthSortPerFrame` ALSO sets each visible normal-mode mesh's `renderOrder` to its content-centroid view-space z (bounding-sphere center through the model-view matrix) every frame — THREE sorts transparent objects by `renderOrder` before `z`, ascending, so the farthest part (most-negative view z) draws first. This runs unconditionally (independent of the within-mesh re-sort hysteresis; it's one cheap matrix-vector per mesh) and is cleared to 0 when a mesh leaves `normal` mode. It's per-_object_ ordering — exact for disjoint convex cells, approximate only where splat footprints spill across a tile boundary (only per-splat / single-mesh sorting is exact there; classical imports use a single-leaf `stream` recipe to get that). Additive/luminous/max are commutative so their order is irrelevant; normal-mode Points/Lines remain a non-goal.
