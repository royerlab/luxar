...
- Samples FPS using a 1-second sliding window, evaluated every 500 ms; a frame gap counts as dead time (window reset, pending probe voided) only when it is BOTH > `gapResetMs` and strictly more than 4× the median of the recent inter-frame intervals, so an isolated stall or idle-resume is still discarded while a genuinely slow cadence is kept and the loop keeps adapting below ~1000/`gapResetMs` fps (a software rasterizer at 0.5 fps used to sit at native DPR forever); the sliding window likewise retains a two-sample minimum so the estimate stays defined slower than the window itself. Up to two dead intervals in a row stay outliers and are discarded; the third IS the 5-interval median and is absorbed as the frame rate, so the window it lands in is treated as unrepresentative for a couple of intervals (scale-downs apply, nothing is learned) and a stall burst can never teach a floor. Residual limitation: a dead period alternating one-for-one with a SINGLE fast frame makes the dead intervals the median, so they are kept as "the frame rate" and a window landing on the fast frame reads as healthy
