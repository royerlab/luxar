...
| `erf.ts`                     | The viewer's single erf source: `erfRef` (A&S 7.1.26, CPU reference) plus the shader polynomial (`erfPoly` TS mirror, `GLSL_ERF_FUNCTIONS`, `erfPolyTSL`) — degree-13, no `exp`/division, generated from one coefficient array; backend agreement is pinned by the `erf` parity fixture and codegen snapshot                                                                                   |
