...
| `line-capsule.ts`            | Capsule line primitive (#1352): the calibration constants (`CAPSULE_SUPPORT_SIGMA`, `CAPSULE_RADIUS_PER_QUAD_HALFWIDTH`, `CAPSULE_JOINT_PACKET_MIN_RADIUS_PX`, …), the sharpness map `capsuleProfileExponent`, and the CPU reference profile + joint-composition model (`capsuleProfile`, `capsuleJointCompositionError`) the unit tests pin — single literal source for the GLSL strings and TSL graphs                                                                                                                                   |
