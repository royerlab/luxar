...
| `glass-partition.ts`         | The refraction split's per-fragment depth partition (spec MESH_PHYSICAL_MATERIALS §3.4 Phase 3): `uGlassPartition` (0 off / 1 keep behind-or-none / 2 keep front) + `uGlassDepth`, the ONE shared `DepthTexture` every data material binds (`getGlassDepthTexture`), the duck-typed `setGlassPartition` helper in the `density-drop.ts` style, and the GLSL uniform + guard chunks every visual fragment shader runs first. Free of `three/tsl` like `erf.ts`                                                                                                                                             |
| `glass-partition-tsl.ts`     | The TSL twin: `glassPartitionNodes` / `glassPartitionNodesFromUniforms`, the clip-`zw` varying the vertex bodies fill (`clipDepthVarying`), `GlassFragmentDepthNode` (window depth from `z/w`, remapped per coordinate system at build time — three's depth nodes read the vertex attribute the data graphs replace) and `glassPartitionGuardTSL`, which samples the depth texture at `screenUV` exactly as `viewportDepthTexture` does                                                                                                                                                                   |
...
| `live-texture-tsl.ts`        | `bindLiveTexture(node, slot, fallback)` — keeps a TSL `texture()` node pointed at a host `IUniform` whose `.value` is swapped between passes (BloomChain walks the mip pyramid; FxaaPass assigns its LDR target per render). The swap runs in `updateBefore`, NOT `.onUpdate('render')`: node `update` callbacks run in graph order, so a texture bound there reaches the sample-tap clones after some have already derived their render-target Y-flip from the previous texture (#2584). Split from `tsl-helpers.ts` because it needs `NodeUpdateType` from `three/webgpu`                               |
...
| **Lines**   | **Per fragment** (quad) / **per corner** (capsule) | None at 0.01 — the fade multiplies into the intensity chain | A segment SPANS depth, so either endpoint's value would be wrong along it. The vertex stage instead culls a both-endpoints-near segment and clips a half-near one onto the plane. The quad variant then evaluates the fade per fragment from the interpolated view depth. The capsule variant (the default primitive) evaluates it per corner at the clamped span parameter and interpolates the result across its over-long stencil — cheaper, with exact fade values only at the stencil's axial extremes; the cap-extension stretch shifts even the drawn endpoint values. Even without that extra stretch, interpolation replaces the smoothstep along the axial span with its chord: a span matching `[nearCull, 2·nearCull]` deviates by at most 0.096, while a span clipped at `nearCull` and ending at `3·nearCull` under-fades by 0.5 at mid-span, and the maximum error approaches the full ramp as the far endpoint recedes (the long-segment case noted in `line/shader-glsl.ts`). Two legs sharing a vertex also disagree on it away from that vertex (the over-fill class `_shared/line-capsule.ts` documents); these are relaxed #1352 approximations. The clip keeps corners out of the behind-eye hard-zero branch, while the ramp reaches zero continuously at `nearCull` and the both-near cull only removes a span already in that zero region, so neither variant has a pop. |
