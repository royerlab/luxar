...
| `erf.ts`                     | The viewer's single erf source: `erfRef` (A&S 7.1.26, CPU reference) plus the shader polynomial (`erfPoly` TS mirror, `GLSL_ERF_FUNCTIONS`, `erfPolyTSL`) — degree-13, no `exp`/division, generated from one coefficient array — and the exponential A&S shader forms (`GLSL_ERF_AS_FUNCTIONS`, `erfAsTSL`) for lanes whose erf error is amplified unboundedly (the volumetric line mixed-end lane); backend agreement is pinned by the `erf` parity fixture and codegen snapshot                                                           |
| `line-volumetric.ts`         | Volumetric line primitive (#1352): the calibration constants (`LINE_SIGMA_PER_WIDTH`, `LINE_STENCIL_DILATION`, `LINE_PARALLEL_LANE_THRESHOLD`) and the quadrature-validated CPU reference of the sum-family ray integral (`lineVolumetricSumIntegral`, `lineVolumetricModelDensity`, `erfCapRemainder`) that both shader backends' fragment lanes mirror — edit the lanes here first, prove them in `line-volumetric-integral.test.ts`, then port                                                                                           |
| `line-capsule.ts`            | Capsule line primitive (#1352): the constants both backends fold (`CAPSULE_RADIUS_PER_QUAD_HALFWIDTH`, `CAPSULE_MIN_RADIUS_PX`, `CAPSULE_STENCIL_APRON_PX`, `CAPSULE_JOINT_DEFICIT_GATE`, `CAPSULE_JOINT_PACKET_MIN_RADIUS_PX`), the CPU reference profile, the joint-end stencil reach rule (`capsuleJointStencilReach` — full disc once a deficit packet exists, #1488), and the CPU joint-composition model (`capsuleJointCompositionError`), which mirrors the fragment math AND the vertex stencil so a short reach chops it           |
| `line-integral-lut.ts`       | Sharpness radial LUT for the volumetric line sum lanes (#1352 PR-4): `S(q, s)` = shifted+normalized untruncated Abel transform of `exp(−K·rad^β)`, `β = 2^(6s−2)`, tabulated 128×65 — the odd height puts the default knob 0.5 exactly on the β = 2 row — (tanh-sinh quadrature CPU reference `lineRadialProfile`) into a lazy-singleton R16F linear-filtered texture (`getLineRadialLUTTexture`). The β = 2 row IS the analytic radial, so the sum fragments sample it unconditionally — no analytic/LUT seam; rows hit exactly 0 at q = 1 |
