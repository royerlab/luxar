...
| `lifecycle.ts` | `subscribeToDispose` wires a `'dispose'` listener that removes the material from every registry (and from the permanently-empty legacy cache maps); `removeFromRegistries` is the same teardown reachable from `MaterialManager.unregister`; `SOFT_DISPOSE_FLAG` is the opt-out sentinel.                                                                      |
