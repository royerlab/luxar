...
| `lifecycle.ts`         | `subscribeToDispose` wires a `'dispose'` listener that removes the material from every registry (and from the permanently-empty legacy cache maps); `removeFromRegistries` is the same teardown reachable from `MaterialManager.unregister`; re-exports the `SOFT_DISPOSE_FLAG` opt-out sentinel.                 |
...
| `stats.ts`             | `getCacheStats(ctx)` — diagnostic snapshot (registry sizes, cumulative `new XMaterial()` wall-clock; the cache/eviction fields survive as always-empty/0 so stats consumers don't break).                                                                                                                         |
