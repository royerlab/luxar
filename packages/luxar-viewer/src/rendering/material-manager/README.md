...
| File           | Role                                                                                                                                                                                                                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `factories.ts` | `VISUAL_FACTORIES`, `PICKING_FACTORIES`, `MEGA_SHADER_FACTORIES` constructor tables keyed by `kind` × `backend`; `resolveMaterialBackend(caps)` (caps.apiSurface → `'glsl' \| 'tsl'`); `lineCacheKey` with shared integer bucketing (point and gsplat materials are per-node and uncached, so they have no cache key); `BlendingMode` type. |
