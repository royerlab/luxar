...
- `Cargo.lock` — Generated by `cargo`/`wasm-pack`; gitignored (see `.gitignore`).
...
- `.gitignore` — Ignores `target/`, `Cargo.lock`, `pkg/` (wasm-pack output goes to `public/wasm/`), and editor backups.
