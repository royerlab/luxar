...
| `gsplats-processing.ts` | Marginal Cholesky factorization, Mahalanobis distance, fused nD→3D projection (`project_gsplats_nd_to_3d`). Frounds every float step in the Rust op order (#1820); bit-exact vs WASM (`exp`/`log` via `float32-math.ts`, #1830) |
| `float32-math.ts`       | Exact TypeScript ports of the Rust/WASM `expf` and `logf` routines, used where host `Math.exp` / `Math.log` would round differently (#1830)                                                                                     |
