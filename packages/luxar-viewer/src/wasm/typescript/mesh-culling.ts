...
export function mesh_vertex_visibility_mask(
  ...
  // runs over `numHidden` — usually 1, a timepoint or channel.
 ...
 * @returns Number of visible faces written (slice `output` to `3 ×` this)
 */
export function compact_visible_faces(
  ...
  for (let f = 0; f < numFaces; f++) {
    const base = f * 3;
    const i0 = faces[base];
    const i1 = faces[base + 1];
    const i2 = faces[base + 2];

    // Range-check before indexing the mask — see the note above.
