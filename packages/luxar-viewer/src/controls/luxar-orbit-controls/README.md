...
- **The two DRIVEN motions bypass the damping accumulators.**
  ...
  rather than feeding `zoomDelta`. Routing either through the damping
  filter would low-pass it — the dolly would fall short of its amplitude
  and lag the period the user asked for. The damped deltas are for USER
  input, which is what wants smoothing.
