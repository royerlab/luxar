...
├── listeners.ts           # attachListeners → disposer (mouse, wheel, optional keyboard)
└── input/                 # DOM-event handler bodies (keyboard, mouse, wheel)
```
...
KeyboardEvent / MouseEvent / WheelEvent
        │
        ▼  (input/keyboard.ts, input/mouse.ts, input/wheel.ts)
...
- [`./input/README.md`](./input/README.md) — the DOM-event handler subfolder (keyboard / mouse / wheel) that produces the `moveState` / `lookState` / `velocity` / `orientation` updates these helpers consume.
