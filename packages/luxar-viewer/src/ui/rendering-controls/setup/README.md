...
- **Density Guard** toggle (`DensityGuardControl.setEnabled`, present when the context carries a `densityGuard` handle) plus a read-only **Thinning** row (`off` / `none · cap C` / `N nodes · keep 1/K · cap C`, refreshed every 500 ms). The toggle is bound to the guard's LIVE state rather than the stored `densityGuardEnabled` flag, because `?no-density-guard` can hold the guard off while the stored flag says on; the flag is written (and saved) only when the session is not URL-disabled.
