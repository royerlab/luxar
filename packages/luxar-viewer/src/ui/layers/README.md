...
- **Ambient** / **Shade falloff** / **Specular** / **Shininess** — **mesh only**, and the first controls here gated on geometry TYPE rather than blending mode. Ambient and shade falloff parameterize the wrapped diffuse term from the fixed view-space light direction; specular and shininess control the additive Blinn–Phong highlight. Hidden for points/lines/gsplats because mesh is the only geometry type with surface normals. The exponent tracks start at the material’s `0.001` clamp rather than 0, avoiding undefined `pow(0, 0)` at an unlit fragment.
