...
- **Depth level** — the authored cross-layer draw order (`docs/guides/specs/LAYER_DEPTH_LEVEL_SPEC.md`). A number field rather than a slider, because the value is an unbounded signed integer and must be able to be **blank**: empty (placeholder `auto`) means _unset_, which hands the layer back to the renderer's inferred ordering — bounding-sphere containment forcing a container to draw before its contents — while any explicit value, **0 included**, states a band and suppresses that inference. Higher draws nearer the camera, like a CSS `z-index`. Layers on different levels never interleave whatever the camera does, which is the whole point: it converts an inferred, geometry-dependent order into a stated one. Sparse values (10/20/30) leave room to insert a layer later. Unlike every other control here the level is not a material uniform but a cross-node **sort key**, so `applyDepthLevel` writes `userData.attrs.depth_level` on each affected leaf and wakes the render loop rather than going through `applyComposed` — there is no `mat.updateX` to call. Two things it cannot do: reorder across the opaque/transparent split (an `opaque` mesh always draws before any transparent layer, and a level spanning both warns), and make interpenetrating concave layers _correct_ — it buys stability, not correctness.
