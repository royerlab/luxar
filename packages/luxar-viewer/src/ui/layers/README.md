...
Edits made in the panel are viewer-only and never written back to the zarr store. They are, however, mirrored into the page URL as `#layers=<url-encoded JSON>` (only the fields that differ from the authored scene, so an untouched scene adds nothing) and applied again when a scene loads, so the address bar is a share link; **Reset all layers** empties the fragment and a dataset switch drops it. The Layers header's right-click menu also offers **Copy layer settings** (pretty JSON to the clipboard), **Download layer settings…** (`layer-settings.json`) and **Load layer settings…** (reset to authored, then apply the file; layers the scene lacks are skipped with a toast). The document is `layer-settings.ts`'s `LayerSettingsDoc` — `{ version: 1, layers: { <path>: LayerPatch } }` — published as `schemas/layer-settings.v1.schema.json`; a unit test pins the schema's field list to the runtime validator, and adding a field to `LayerPatch` fails to compile until both are updated. The URL mirror is opt-in (`LuxarApp.bindUrl` via the standalone bootstrap's `updateBrowserUrl`), so an embedded viewer never touches its host page's URL.
...
layer-settings.ts  LayerSettingsDoc — diff vs authored, JSON parse/validate, #layers= hash codec + debounced URL writer
```
...
| `layer-settings.ts`                        | `LayerSettingsDoc` — only-what-changed diff, JSON parse/validate, `#layers=` hash codec, debounced URL writer (no DOM)            |
| `../../../schemas/layer-settings.v1.schema.json` | Published JSON Schema of the document                                                                                      |
