...
- **Glass themes** (`frostedGlassTheme`, `liquidGlassTheme`) are intended to overlay arbitrary canvas content, so each keeps its own contrast floor rather than sharing one background shape: `frostedGlassTheme.colors.background.secondary` is a translucent **dark** frost (`rgba(28, 30, 36, 0.75)`), while `liquidGlassTheme.colors.background.secondary` is a translucent **white** (`rgba(255, 255, 255, 0.15)`) that never paints on a `.luxar-glass-surface` at all — that class sets `background: transparent` in `styles/themes/liquid-glass.css`, so the token survives only on non-glass popovers, where a separate dark tint (`--luxar-glass-tint`, also in that file) now replaces it outright as the panel's actual contrast floor; on glass surfaces themselves, the translucent-white inner lifts that _are_ painted (e.g. `background.tertiary`) sit ON TOP of that tint, not under it. They share an Apple-system font stack and cubic-bezier transitions.
