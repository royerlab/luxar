...
def create_sharpness_gradient_example(scene: Scene, n_points: int = 5000) -> None:
    ...
    scene.add_points("SharpnessGradient", positions, colors, radii=radii, sharpness=sharpness)
...
def create_sharpness_comparison_example(scene: Scene) -> None:
    ...
    labels = ["Very Soft (0.5)", "Linear (1.0)", "Quadratic (2.0)", "Quartic (4.0)", "Very Sharp (10.0)"]
    ...
    for i, (sharp_val, label) in enumerate(zip(sharpness_values, labels)):
        ...
        scene.add_points(f"Sharpness_{label}", positions, colors, radii=radii, sharpness=sharpness)
...
def create_mixed_sharpness_example(scene: Scene, n_points: int = 3000) -> None:
    ...
    # 1/3 sharp points
    sharpness[2*n_points//3:] = rng.uniform(6.0, 10.0, n_points - 2*n_points//3)
    ...
    colors[:, 1] = (128 * (1 - np.abs(normalized_sharp - 0.5) * 2)).astype(np.uint8)  # Green for medium
    ...
    scene.add_points("MixedSharpnessCloud", positions, colors, radii=radii, sharpness=sharpness)
...
def create_sharpness_wave_example(scene: Scene, n_points: int = 4000) -> None:
    ...
    scene.add_points("SharpnessWave", positions, colors, radii=radii, sharpness=sharpness)
...
def main():
    ...
    aprint("- Bottom left: Mixed cloud with soft blue, medium green, and sharp red points")
