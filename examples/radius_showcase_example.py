...
def create_layered_spheres_example(scene: Scene, n_layers: int = 5, points_per_layer: int = 1000) -> None:
    ...
    for i in range(n_layers):
        ...
        colors = np.tile([int(r * 255), int(g * 255), int(b * 255)], (points_per_layer, 1)).astype(np.uint8)
...
def main():
    ...
    output_path = Path("radius_showcase_example.zarr")
    
    aprint(f"Creating radius showcase example at {output_path}")
    aprint("This example showcases the per-point radius feature with various visualizations:")
