...
from luxar import Dimension, Dimensions, Scene, transforms


def main():
    """Create a 5D scene with slider navigation."""
    output_path = Path("dimension_sliders_5d_example.zarr")
    ...
    aprint(f"- Total dimensions: 5 (X, Y, Z, Time, Channel)")
    aprint(f"- Displayed dimensions: X, Y, Z")
    aprint(f"- Navigation dimensions: Time (0-10s), Channel (0-2)")
