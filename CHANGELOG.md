...
sub-group — is #1422 and has not landed. A Lines ladder is a separate gap: #1424
gave lines the per-node segment→vertex chain, but nothing composes the LEVELS of a
ladder the way `PointsProgressiveLoader` now does. Gsplat ladders carry no labels
at all.
