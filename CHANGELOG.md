...
- On the GSplats additive ladder (shared here via `lod_breakpoints.py`), energy-
  fraction breakpoints (a `list[float]` of cumulative fractions) now place their
  cuts on the same O(N) self-energy cumulative the viewer reads back as
  `lod_stats.energy_fraction_cum`, for every score-ordered method (`self_energy`
  / `mass` / `amplitude` / `random`, and `method=auto` above N=5000). Cuts land
  where the on-disk energy stamp reports the requested fraction, and the coarse
  maximally-overlapping levels skip the O(nnz) sparse-Gram build entirely; only
  `greedy` / spectral orderings still cut on the residual-energy curve they
  already build a Gram for. Previously the score-ordered path built a Gram just
  to cut on a residual curve the viewer never sees.
