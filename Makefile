...
# The root README embeds a social-preview banner, an architecture diagram (dark
# and light) and two looping WebP screen recordings. They are generated by the
# scripts below and published by content hash; git carries only the scripts.
# Source clips for the animations are the release social kit on the shared
# drive (LUXAR_SOCIAL_KIT=<.../social_media_release/clips/x_threads_linkedin_1080p>).
...
generate-readme-diagram:  ## Draw the "how Luxar works" diagram, dark + light PNG (needs rsvg-convert)
...
publish-readme-media:  ## Upload build/readme-assets/* to data.luxarviewer.dev/media by content hash (rclone r2:), verify, print URLs
	hatch run python scripts/gallery/publish_media.py $(README_ASSETS_DIR)/*.png $(README_ASSETS_DIR)/*.webp
