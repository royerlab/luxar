...
multiset rather than trusting or distrusting a `line_type`: each component's
undirected edges, including duplicate multiplicity, must be exactly its
consecutive-vertex pairs. Qualifying data can now carry the
ladder; under `substitutive_lod=` it is enabled by default unless explicitly
disabled. `cosmicflows_laniakea` uses it, while `dmri_tractography` explicitly
...
chord, or duplicate edge) is refused: a `UserWarning` under a substitutive
wrapper where a flat level is a reasonable fallback, and a `ValueError` on the
direct path when the resolved ladder has multiple levels. A one-level result
falls through to the flat writer and preserves the authored edges. Partition
preflight remains deliberately eager so invalid indexed input fails before any
part is written.
