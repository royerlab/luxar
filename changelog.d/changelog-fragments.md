...
`make changelog` folds them into `CHANGELOG.md` under the current month at
release-prep, deleting the fragments. A PR now only ever touches its own new file, so
