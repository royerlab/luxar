...
so nobody mistakes debt for a pattern. The five modernization tranches it
catalogued have since landed — a11y hardening (#1476), emoji→stroke icons
(#1479), the dataset browser (#1472), the accent migration plus the shared
panel-header/close recipes (#1480), and token hygiene (#1478) — so what remains
is the recording stop-confirm dialog, the scene-identity banner's inline styles,
the GUI library's raw control fills, the toast's opacity fade on a glass root,
sub-AA contrast on the glass themes' micro-label ranks, and one modal that
doesn't yet trap focus.
