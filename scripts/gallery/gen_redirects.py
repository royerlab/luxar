...
``/d/<demo-key>`` is an identity; each store's dated prefix is a location. This
maps the former onto the latter and is regenerated at deploy, so the mapping
updates itself and cannot drift from what is served. Targets use the standalone
viewer and percent-encode the source URL to match the deployed redirects.
...
def normalise_stores(names: Iterable[str], default_prefix: str) -> dict[str, str]:
    ...
    for raw in names:
        ...
        parts = name.split("/")
        store_index = next(
            (index for index, part in enumerate(parts) if part.endswith(".luxar.zarr")),
            len(parts) - 1,
        )
...
def render(lines: Sequence[str], prefixes: Iterable[str]) -> str:
    prefix_list = ", ".join(sorted(set(prefixes)))
    header = [
        "# Stable per-demo routes. GENERATED by scripts/gallery/gen_redirects.py",
        "# at deploy time — do not hand-edit; it is rewritten with the live data",
        "# prefixes on every publish.",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--prefix",
        required=True,
        help="default dated data prefix for bare --live-stores entries",
    )
    parser.add_argument(
        "--live-stores",
        required=True,
        help="file listing bare stores or <prefix>/<store> entries, or '-' for stdin",
    )
    ...
    live = normalise_stores(text.splitlines(), args.prefix)
    if not live:
        raise RouteError(
