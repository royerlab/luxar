...
| `audit_readme_demo_count.py` | Report-only: does the README's stated live-demo count match the gallery being deployed? Runs at deploy time, because that is when the fact changes. Never blocks — a stale README must not stop correct data reaching the site. `../sync_demo_counts.py` owns the README's hosted-count claims (projected from `manifest.json` and CI-gated by `hatch run check-demo-counts`); this audit reads the same claim table and adds the one comparison CI cannot make — against the tiles actually in the built page. |
