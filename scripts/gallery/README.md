...
| `check_tile_staleness.py` | Report-only check for committed README tiles older than their manifest-listed demo generator, `luxar.shading` production code, or their own manifest entry. Uses entry-specific blame so editing one demo does not mark every tile stale. |
...
It compares commit timestamps for each committed `.webp` against only the
manifest-listed demo generator, production Python files directly under
`packages/luxar/src/luxar/shading/`, and the lines of that demo's own manifest
object. Shading tests/docs and shared viewer paths are excluded: they do not
change the rendered tile or change too often to provide useful signal. The
check refuses shallow clones rather than silently producing incomplete history;
