...
| `gen_redirects.py` | Generates the demo site's stable `/d/<demo-key>` routes as a Cloudflare Pages `_redirects`. **Load-bearing**: the root README links 29 tile titles at these routes, and `--check-contract` fails the build naming any linked key that lost one. Takes the store from each entry's `dataset`, never its `id` — 8 of 90 differ, and a mechanical mapping emits routes to stores that do not exist (which the origin answers `200 text/html`, i.e. a blank viewer, not a 404). Regenerate on **every** deploy so the dated prefix inside stays current. |
