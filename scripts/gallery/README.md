...
| `generate_gallery_datasets.py` | Generates each demo's `.luxar.zarr` under `datasets/demos/` (idempotent; skips ones already present; best-effort). A demo whose `DEMO_META` declares an un-fetchable `local_data` (`manual-file` / `kaggle-auth`) is still run, but a **non-zero exit** is reported in the soft `manual-data` bucket instead of failing the build — that input may simply not exist on this machine. A `timeout` or a `no-output` stays hard, so one case remains: `arxiv_papers_kaggle` starts without credentials and downloads ~30 GB, which on a cold machine can exhaust the per-demo timeout and still return 1. |
