...
| `check_tile_staleness.py` | Report-only check for committed README tiles older than their manifest-listed demo generator, `luxar.shading`, or their own manifest entry. Uses entry-specific blame so editing one demo does not mark every tile stale. |
...
manifest-listed demo generator, `packages/luxar/src/luxar/shading/`, and the
lines of that demo's own manifest object. Shared viewer paths are excluded: they
change too often to provide useful signal. The check refuses shallow clones
rather than silently producing incomplete history; run it from a full checkout.
