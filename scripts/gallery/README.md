...
| `gen_redirects.py`                                                           | Generates the demo site's stable `/d/<demo-key>` routes as a Cloudflare Pages `_redirects`. **Load-bearing**: the root README links 29 tile titles at these routes, and `--check-contract` fails the build naming any linked key that lost one. Takes the store from each entry's `dataset`, never its `id` — 7 of 88 differ, and a mechanical mapping emits routes to stores that do not exist (which the origin answers `200 text/html`, i.e. a blank viewer, not a 404). Regenerate on **every** deploy from the bounded recursive live-store listing so each route keeps the prefix where its store is live.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `audit_readme_demo_count.py`                                                 | Report-only: does the README's stated live-demo count match the gallery being deployed? Runs at deploy time, because that is when the fact changes. Never blocks — a stale README must not stop correct data reaching the site. Nothing else watches this number (`sync_demo_counts.py` owns the _bundled_ count) and it has drifted twice.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- **Auto-exposure:** measures the composited frame (a Playwright screenshot,
  ...
  just below clipping; then, if the lit histogram turns out to be _narrow_
  (p99 − p10 < `NARROW_SPREAD_MAX`, where p99 says nothing about where the
  subject sits), re-target the lit _median_ to
...
- **Crop check:** counts the **lit pixels on the frame's outermost row/column**
  ...
  loop is blind to it _by construction_: its 3rd–97th-percentile bbox discards
  ...
  **camera's own up-vector** most nearly points along — so a scene that bakes
  ...
  structure) into a VP9 WebM master and a smaller animated WebP for the README.
    ...
    **Timelapse demos need the rAF loop re-frozen every frame.** The orbit freezes
    ...
    _after_ the per-frame `page.evaluate` returns, so re-applying the pose inside
...
recorded", which is deliberately _not_ the same claim as "nothing to credit", so
...
The usual cause is the data server binding a _different_ port than the one
