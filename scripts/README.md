...
| `check_cadence_liveness.py` | Query GitHub Actions metadata with an `actions: read` token and fail when one of the *workflow* cadences listed in its `CADENCES` table has gone stale, is disabled, or is missing from the Actions listing at or after its `not_before` date (never promoted to `main`, deleted, or renamed out from under the table — the listing is effectively default-branch-scoped, so absence tracks promotion rather than the cadence). Before `not_before`, an absent workflow and a cadence that has never succeeded both report the non-failing `NOTICE`. Successes are all it reads, so a cadence that fires daily and *fails* looks the same as one that stopped firing; `cuda-nightly.yml` files an issue on failure, which is the control for that. Only workflow cadences can be covered — a `workflow_runs` query structurally cannot see a systemd timer on a host — and detection latency would be set by how often a host job runs this script (none does yet), which must be at least as often as the smallest `max_age` in the table. Weather (a connection error, a truncated / non-JSON / non-UTF-8 body, HTTP 408, 425, 429, any 5xx, and a 403 carrying rate-limit headers) is retried once and then reports the non-failing `UNKNOWN`; a permanent status (401, 404, a 403 without rate-limit headers — typically a missing `actions: read` scope) and a malformed payload are red |
